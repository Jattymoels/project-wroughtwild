...
func check(ok: bool,label: String) -> void:
 if not ok: failures+=1; printerr("FAIL RF02 WALK: ",label)
...
func _walk() -> void:
 ...
 check(terrain._rf02_biome_mask!=null and terrain.world_profile()=="frontier_v6","ordinary seed/class New World installs RF02")
 ...
 player.spring_arm.rotation.x=-0.17
 ...
 check(SaveManager.new().write(output.path_join("new-world-after-walk.json"),player),"ordinary New World saves after the walk")
 ...
 print("RF02_WALK distance=",distance," frames=",frames," captured=",captured," failures=",failures)
 _finish()
