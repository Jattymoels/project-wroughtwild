extends RefCounted
## Production presentation adapter. Original meshes, bodies, poses and refresh bounds stay authoritative.
...
func material_for(source: Material, struck: bool) -> Material:
	...
	# Equal textures may still have distinct tint or ORM inputs.
	var key:Array=[base.albedo_texture,base.albedo_color,base.metallic,base.roughness,scarred,base.roughness_texture]
...
func install(world: Node3D) -> void:
	var history: CataclysmSites = world.get_node_or_null("CataclysmSites")
	var scar_assigned := false
	for part: Node3D in (history.pieces if history != null else []):
