...
func _ready() -> void:
 ...
 restore_recovery.call_deferred()
func restore_recovery() -> void:
 ...
 check(hash([terrain.map.heights,terrain.map.impacts,terrain.map.leylines,terrain.map.nodes])==int(expected.native_digest),"exact native exposure, source identity and terrain")
