...
## One short ordinary walk across the diagnosed grassland slope to the paid overlook.
...
func _walk() -> void:
 ...
 # Same used approach as RF03, starting beside the diagnosed small patch.
 var route:=PackedVector3Array([Vector3(555.5,terrain.height_at(555,511),511.5),Vector3(582.5,0,511.5),Vector3(607.5,0,511.5)])
 ...
 player.rotation.y=0
 player.spring_arm.rotation.x=-.18
 player.set_physics_process(true)
 set_physics_process(true)
 Engine.max_fps=60
 for i in 90: await get_tree().process_frame
 check(player.is_on_floor(),"meadow approach starts with real floor contact")
 await still("01-meadow-landform.png")
 ...
 while segment<route.size() and frames<1200:
  ...
  if distance>8 and captured<120 and frames%4==0:
   await RenderingServer.frame_post_draw
   get_viewport().get_texture().get_image().save_jpg(output.path_join("media/walk-%03d.jpg"%captured),.90)
   captured+=1
  if not meadow_still and distance>18:
   player.spring_arm.rotation.x=-.42
   await get_tree().process_frame
   await still("02-local-ground.png")
   player.spring_arm.rotation.x=-.18
   meadow_still=true
  if stuck>=120:break
 ...
 check(segment==route.size(),"continuous affected-patch-to-overlook route completed")
 check(distance>50,"ordinary walk crosses measured slope into the retained paid home")
 ...
 var report:={"checks":checks,"failures":failures,"profile":world_profile,"seed":world_seed,"distance_m":distance,"controller_frames":frames,"captured_frames":captured,"clip_fps":15,"route_complete":segment==route.size(),"stuck_frames":stuck,"start":[begin.x,begin.y,begin.z],"end":[player.position.x,player.position.y,player.position.z],"rows":walk_rows,"scope":"One daylight Forward+ walk across the diagnosed patch, initial pose at (555.5, native height, 511.5); ordinary controller/world work with no jumps or teleports afterward. Same broad rise and paid overlook home."}
 FileAccess.open(output.path_join("walk-checks.json"),FileAccess.WRITE).store_string(JSON.stringify(report,"\t"))
 print("RF04_WALK distance=",distance," frames=",frames," captures=",captured," failures=",failures)
 end_walk()
