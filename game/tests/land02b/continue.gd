...
func restore_run() -> void:
 ...
 check(world_profile=="frontier_v10" and world_seed==77,"saved identity overrides fresh seed/profile choice")
 ...
 var floor_at:=scar+direction*2.15
 floor_at.y=terrain.height_at(floori(floor_at.x),floori(floor_at.z))
 var lip:=scar-direction*5
 var lip_height:=terrain.height_at(floori(lip.x),floori(lip.z))
 ready_at(floor_at+Vector3.UP*2);terrain.set_process(true)
 for i in 25:await tick()
 check(player.is_on_floor() and player.position.y<lip_height-1,"restored native recess supplies real player contact below its lip")
 ...
 quiet()
 # One V9 fixture checks changed dispatch/guards; immutable generation matrices
 # and prior gameplay evidence are deliberately not replayed.
 var older:=WroughtwildSim.new()
 check(older.load_tuning(load("res://scripts/sim.gd").get_tuning_directory()) and older.set_world_profile("frontier_v9"),"existing V9 still loads")
 var old_map: Dictionary=older.world_map(77)
 check(hash(old_map.blocks)==2464639526 and hash(old_map.heights)==2471001676,"V9 geography matches retained LAND-02 evidence exactly")
 older.contraption_bind_world("frontier_v9",77);older.leyline_bind_world("frontier_v9",77)
 var old_save: Dictionary={"schema_version":SaveManager.SCHEMA_VERSION,"world_seed":77,"world_profile":"frontier_v9","sim":older.export_json(),"contraptions":older.contraption_save(),"leylines":older.leyline_save(),"player":{"position":[old_map.spawn_x,old_map.heights[int(old_map.spawn_z)*int(old_map.width)+int(old_map.spawn_x)]+1,old_map.spawn_z],"yaw":0,"pitch":0},"blocks":[],"stations":[],"resource_nodes":[],"world_drops":{"version":1,"pickups":[],"bundles":[]}}
 check(manager.apply(player,old_save),"one existing-profile save restores: "+manager.last_error)
 quiet()
 check(world_profile=="frontier_v9" and terrain.map.get("scarwater",[]).size()==1 and hash(terrain.map.heights)==hash(old_map.heights),"V9 Continue keeps V9 geography with no V10 migration")
 check(_sim().contraption_save()==older.contraption_save(),"V9 finite owner preserved")
 finish_restore()
func finish_restore() -> void:
 ...
 print("LAND02B_CONTINUE ",checks," checks / ",failures," failures")
