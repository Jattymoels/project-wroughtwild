...
func run() -> void:
 ...
 check(world_profile=="frontier_v11" and world_seed==77,"Continue selects saved profile/seed")
 check(hash(terrain.map.heights)==int(expected.height_hash) and hash(terrain.map.dry_steppe[0])==int(expected.place_hash),"exact Steppe geometry and relationships reconstruct")
 ...
 var blue:=fixture("blue_delay");var buffer:=fixture("red_heat_buffer")
 check(machine(blue).pending_request and machine(blue).delay_paused and machine(buffer).heat==1,"paused fractional request and paid heat restore")
 var ledger:=_sim().contraption_save();blue._physics_process(100)
 check(_sim().contraption_save()==ledger,"restart gives paused device no offline work")
 var source_ledger:=_sim().leyline_save()
 check(_sim().world_effect_active("host_defeated:"+String(expected.host_id)),"finite Red death ownership restores")
 for pack: Dictionary in mob_packs.packs:
  if pack.get("frontier_host_id","")==expected.host_id:
   mob_packs._spawn_pack(pack,mob_packs.pack_position(pack));check(pack.members.is_empty(),"Continue cannot respawn finite Red host")
 ...
 bad=saved.duplicate(true);bad.world_profile="frontier_v10";corruptions.append(bad)
 ...
 for rejected: Dictionary in corruptions:
  check(not manager.apply(player,rejected) and _sim().leyline_save()==source_ledger and _sim().contraption_save()==ledger and _sim().export_json()==saved.sim and world_profile=="frontier_v11","invalid/missing/wrong-world owners reject before live mutation")
 # One V10 and one LF payload exercise changed restore dispatch only. Earlier
 # generation/campaign/body evidence is reused; no full old-profile play replay.
 for profile in ["frontier_v10","living_frontier_wave3"]:
  ...
  if profile=="living_frontier_wave3":older.leyline_work("red_home_margin")
