...
func _ready() -> void:
 output=OS.get_environment("WROUGHTWILD_RF06_OUTPUT")
...
func run_fen() -> void:
 ...
 var root: Vector3=plants[0].pose.origin
 ...
 for row in wet_rows():
  ...
  if floori(p.x)==dug.x and floori(p.z)==dug.z:hanging=true
 check(not hanging,"refresh leaves no floating plant or cave-floor projection")
 ...
 check(SaveManager.new().write(output.path_join("private-world.json"),player),"private existing-world checkpoint written")
...
func finish_rf(job: String) -> void:
 ...
 print("RF06_",job.to_upper()," ",JSON.stringify(report))
