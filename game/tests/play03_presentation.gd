...
class EntryControls extends WorldSeedControls:
	var probe: Node
	func release() -> void:
		probe.check(FinishedFauna.resources_ready() and not player.is_physics_processing(), "production resource preparation completes before New World controls release")
		super.release()
...
func _run() -> void:
	...
	check(FinishedFauna.resources_ready() and not chosen.spawned, "production resources ready with selected pack still dormant")
	...
	check(FinishedFauna.resources_ready() and not chosen.spawned, "prepared resources do not activate the pack early")
	...
	finish()
func finish() -> void:
	trace.stop("presentation_arrival_complete")
