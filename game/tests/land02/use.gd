...
func finish_use() -> void:
 ...
 print("LAND02_USE ",checks," checks / ",failures," failures, walked ",route_distance," m")
...
func walk_route(key: String) -> bool:
 ...
 for at: Vector3 in targets:
  if not await travel(at,170):
   print("LAND02_ROUTE_STOP ",key," at=",at," player=",player.position)
   for contact_index in player.get_slide_collision_count():
    var contact:=player.get_slide_collision(contact_index)
    print("LAND02_CONTACT ",contact.get_collider().get_path()," point=",contact.get_position()," normal=",contact.get_normal())
   ok=false;break
 print("LAND02_ROUTE ",key," reached=",ok," distance=",route_distance)
...
func use_run() -> void:
 ...
 check(world_profile=="frontier_v9" and world_seed==77,"normal New World selects V9")
 ...
 check(preload("res://land02/kit.gd").ready,"shared art prepared before controls release")
 ...
 print("LAND02_USE paid homes complete")
 ...
 var scar: Vector3=place_data.scar
 ...
 face_at(scar+Vector3.UP*.2);await still("scar.png")
 quiet()
 ...
 var cell:=Vector3i(floori(scar.x),int(scar.y)-1,floori(scar.z))
