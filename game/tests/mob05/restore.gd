extends Sandpit
## One real generated Bog Lurker pack through ordinary SaveManager/Continue.
var checks := 0
var failures := 0
const SAVE := "user://nymph-continue.json"
func check(ok: bool, label: String) -> void:
	checks += 1
	if not ok:
		failures += 1
		printerr("FAIL NYMPH CONTINUE: ",label)
func _ready() -> void:
	world_seed = 77
...
func pack_for_nymph() -> Dictionary:
	for pack: Dictionary in mob_packs.packs:
		if "bog_lurker" in pack.enemies: return pack
	return {}
func nymph_for(pack: Dictionary) -> Enemy:
	for member in pack.members:
		if is_instance_valid(member) and member.enemy_id == &"bog_lurker": return member
	return null
func _run() -> void:
	var pack := pack_for_nymph()
	check(not pack.is_empty(),"ordinary seed 77 contains native Bog Lurker")
	...
	quiet()
	var enemy := nymph_for(pack)
	check(enemy != null and (enemy._mesh.get_node("Motion") as CreatureMotion).finished is NymphPresentation,"generated native identity selects nymph")
	...
	if not pack.spawned: mob_packs._spawn_pack(pack,point)
	quiet()
	enemy = nymph_for(pack)
	check(enemy != null and (enemy._mesh.get_node("Motion") as CreatureMotion).finished is NymphPresentation,"Continue instantiates native nymph")
	if enemy != null:
	...
	var evidence := FileAccess.open(OS.get_environment("WROUGHTWILD_NYMPH_OUTPUT")+"/restore-checks.json",FileAccess.WRITE)
	...
	print("NYMPH_CONTINUE %d checks, %d failures" % [checks,failures])
