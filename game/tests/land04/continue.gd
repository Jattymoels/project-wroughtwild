...
func run() -> void:
 ...
 check(world_profile=="frontier_v12" and world_seed==77,"Continue selects saved profile/seed")
 check(hash(terrain.map.heights)==int(expected.height_hash) and hash(terrain.map.force_journeys)==int(expected.journey_hash),"exact V12 terrain and force/host relationships reconstruct")
 ...
 bad=saved.duplicate(true);bad.world_profile="frontier_v11";corruptions.append(bad)
 ...
 bad=saved.duplicate(true);payload=JSON.parse_string(saved.leylines);payload.profile="frontier_v11";bad.leylines=JSON.stringify(payload);corruptions.append(bad)
 bad=saved.duplicate(true);payload=JSON.parse_string(saved.contraptions);payload.world_profile="frontier_v11";bad.contraptions=JSON.stringify(payload);corruptions.append(bad)
 for rejected: Dictionary in corruptions:
  check(not manager.apply(player,rejected) and _sim().leyline_save()==source_ledger and _sim().contraption_save()==ledger and _sim().export_json()==saved.sim and world_profile=="frontier_v12","invalid/missing/wrong-world owners reject before live mutation")
 # One V11 and one LF payload exercise changed restore dispatch only. Earlier
 # generation/campaign/body evidence is reused; no full old-profile play replay.
 for profile in ["frontier_v11","living_frontier_wave3"]:
