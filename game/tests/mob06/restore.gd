extends Sandpit
## One real generated Hollow Knight pack through ordinary SaveManager/Continue.
var checks := 0
var failures := 0
const SAVE := "user://tortoise-continue.json"
func check(ok: bool, label: String) -> void:
	checks += 1
	if not ok:
		failures += 1
		printerr("FAIL TORTOISE CONTINUE: ",label)
func _ready() -> void:
	world_seed = 77
...
func pack_for_tortoise() -> Dictionary:
	for pack: Dictionary in mob_packs.packs:
		if "hollow_knight" in pack.enemies: return pack
	return {}
func tortoise_for(pack: Dictionary) -> Enemy:
	for member in pack.members:
		if is_instance_valid(member) and member.enemy_id == &"hollow_knight": return member
	return null
func _run() -> void:
	var pack := pack_for_tortoise()
	check(not pack.is_empty(),"ordinary seed 77 contains native Hollow Knight")
	...
	quiet()
	var enemy := tortoise_for(pack)
	check(enemy != null and (enemy._mesh.get_node("Motion") as CreatureMotion).finished is TortoisePresentation,"generated native identity selects tortoise")
	...
	# Match SaveManager full-precision JSON, including spatial doubles. Default
	# stringify rounds nonempty drop fields; retain every bit and ownership field.
	check(JSON.parse_string(JSON.stringify(WorldDrops.capture(self), "", true, true)) == saved.world_drops,"Continue preserves native loose-drop snapshot including empty outcomes")
	...
	pack = pack_for_tortoise()
	point = mob_packs.pack_position(pack)
	if not pack.spawned: mob_packs._spawn_pack(pack,point)
	quiet()
	enemy = tortoise_for(pack)
	check(enemy != null and (enemy._mesh.get_node("Motion") as CreatureMotion).finished is TortoisePresentation,"Continue instantiates native tortoise")
	if enemy != null:
	...
	var evidence := FileAccess.open(OS.get_environment("WROUGHTWILD_TORTOISE_OUTPUT")+"/restore-checks.json",FileAccess.WRITE)
	...
	print("TORTOISE_CONTINUE %d checks, %d failures" % [checks,failures])
