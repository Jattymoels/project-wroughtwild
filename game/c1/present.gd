...
func install(model:Node3D,id:String,conform:=false,own:=false)->Array:
	...
	for mesh:MeshInstance3D in model.find_children("*","MeshInstance3D",true,false):
		for s in mesh.mesh.get_surface_count():
			...
			# Preserve every shader input while sharing equivalent texture objects.
			var key:Array=[role,conform,old.resource_name,image,old.albedo_color,old.vertex_color_use_as_albedo,old.roughness_texture]
			if not materials.has(key):
				var m:=ShaderMaterial.new();m.shader=R2Resources.resource(get_script().resource_path.get_base_dir()+"/surface.gdshader")
				...
				if role==5:m.set_shader_parameter("textured",true);m.set_shader_parameter("albedo_map",R2Resources.resource(get_script().resource_path.get_base_dir()+"/forest-floor.png"))
				materials[key]=m
