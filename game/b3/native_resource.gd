...
func reproject()->void:
	...
	# Exact call-local samples: no retained terrain result can outlive a rebuild.
		...
		for s in source.get_surface_count():
			...
			for t in range(0,indices.size(),3):
				...
				for k in 3:
					...
					var y:=_r2_height(terrain,r2_samples,position.x+p.x,position.z+p.z)
				...
				for sample in [(points[0]+points[1]+points[2])/3.0,(points[0]+points[1])*.5,(points[1]+points[2])*.5,(points[2]+points[0])*.5]:
					if not is_finite(_r2_height(terrain,r2_samples,position.x+sample.x,position.z+sample.z)):valid=false
				if not valid:continue
