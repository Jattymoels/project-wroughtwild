...
const SOURCES := {
	"wood": ["Ordinary timber trees.", "Work the trunk with {interact}, then collect the freed drop. Recipes naming wood need this wood, not pine or other timber species."],
	"fieldstone": ["Loose field boulders.", "Work the boulder with {interact} and collect the freed pieces. This is rough stone, not dressed building stone."],
	"split_stone": ["Stone seams or cracked ground stone.", "For a seam, set a timber wedge with {interact}, then drive it with {interact}; compatible impact can speed the work. Collect the freed drop."],
	"iron_ore": ["Exposed iron deposits.", "Work the deposit with {interact}, then collect its ore. Iron needs no heat before gathering."]
}
...
static func describe(sim: WroughtwildSim, id: String) -> Dictionary:
	...
	if not rare.is_empty():
		...
		result.work = " → ".join(rare.harvest_stages)+". Use {interact} to work it, then collect the freed component."
		...
		var where := "by hand ({hand_craft})" if station_id == "" else "at the " + String(sim.station(station_id).get("display_name", Hud.pretty(station_id)))
	...
	if id == "timber_wedge":
		...
		result.use = "Set it in a stone seam with {interact}, then drive it with {interact} or a compatible impact."
	...
	if station != "":
		result.shape = id
		result.use = "Place the kit with {toggle_build_mode} → {cycle_shape} → {primary_action}, then use {interact} at the station."
	if sim.fuels().has(id):
		result.use += " Fuel: %d heat per unit; recipe ingredients are reserved first." % int(sim.fuels()[id])
	for field in ["source", "work", "use"]: result[field] = InputPrompts.text(result[field])
	return result
