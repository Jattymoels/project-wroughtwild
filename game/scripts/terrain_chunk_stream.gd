...
func _build_horizon() -> void:
	...
	var step:=4 if terrain.world_profile() in ["frontier_v9","frontier_v10"] else int(_settings.terrain_far_step_cells)
	...
	if terrain.world_profile() in ["frontier_v9","frontier_v10"]:
		for iz in rows-1:
			for ix in columns-1:
				var a:=iz*columns+ix
	...
	if terrain.world_profile()=="frontier_v10":
