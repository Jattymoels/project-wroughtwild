...
static func build(root: Node3D, ground: Terrain) -> void:
	...
	if old!=null:
		...
		old.queue_free()
	if ground.world_profile() not in ["frontier_v5", "frontier_v6","frontier_v7","frontier_v8","frontier_v9","frontier_v10","frontier_v11","frontier_v12","frontier_v13","living_frontier_wave1","living_frontier_wave3"]:return
