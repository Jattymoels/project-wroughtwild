...
static func refresh_buildings(root: Node3D, terrain: Terrain) -> void:
	...
	if terrain.world_profile() in ["frontier_v3","frontier_v4","frontier_v5"]:
...
static func refresh_cover_chunk(terrain: Terrain, chunk: Node3D) -> void:
	if terrain==null or chunk==null or terrain.world_profile() not in ["frontier_v3","frontier_v4","frontier_v5"]: return
...
static func _plant(terrain: Terrain, batches: Dictionary, kind: String, at: Vector3, size: Vector3,
	...
	if terrain.world_profile() in ["frontier_v4", "frontier_v5"] and at.is_finite():
