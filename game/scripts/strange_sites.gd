...
static func refresh_buildings(root: Node3D, terrain: Terrain, changed: Array[AABB] = []) -> void:
	...
	if terrain.world_profile() in ["frontier_v3","frontier_v4","frontier_v5","frontier_v6","frontier_v7","frontier_v8","frontier_v9","living_frontier_wave1","living_frontier_wave3"]:
...
static func refresh_cover_chunk(terrain: Terrain, chunk: Node3D) -> void:
	if terrain==null or chunk==null or terrain.world_profile() not in ["frontier_v3","frontier_v4","frontier_v5","frontier_v6","frontier_v7","frontier_v8","frontier_v9","living_frontier_wave1","living_frontier_wave3"]: return
	var buildings:=_building_index(terrain)
	var rf_buildings: Dictionary=terrain.reclaimed_cover.workspace_index(terrain,buildings) if terrain.reclaimed_cover!=null else {}
...
static func _plant(terrain: Terrain, batches: Dictionary, kind: String, at: Vector3, size: Vector3,
	...
	if terrain.world_profile() in ["frontier_v4", "frontier_v5", "frontier_v6","frontier_v7","frontier_v8","frontier_v9","living_frontier_wave1","living_frontier_wave3"] and at.is_finite():
