class_name FrostOrb
extends Node3D
## The grammar-spike projectile (skill-grammar.md): an orb that flies until
## it meets an enemy, deals sim damage, applies sim chill, then FORKS -
## spawning child orbs at the impact aimed at the nearest untouched enemies.
## The sim owns every number (damage, chill, fork count, decay per
## generation); this node owns flight, collision and who a fork jumps to
## (ADR-0003).

const SKILL_ID := &"prototype_frost_orb"
...
static func launch(from_combat: PlayerCombat, root: Node, from: Vector3,
		dir: Vector3, in_generation: int, in_visited: Array) -> FrostOrb:
	var orb := FrostOrb.new()
	orb.combat = from_combat
	orb.direction = dir.normalized()
	orb.generation = in_generation
	orb.visited = in_visited
	root.add_child(orb)
...
func _ready() -> void:
	...
	material.albedo_color = Color(0.55, 0.8, 1.0)
...
func _hit(enemy: Enemy) -> void:
	...
	var damage: float = combat.sim.player_hit_damage(String(SKILL_ID), combat.alive_enemies().size() == 1)
	damage *= combat.sim.fork_damage_fraction(String(SKILL_ID), generation)
	combat.last_hit_dealt = damage
	enemy.apply_chill(combat.sim.chill_applied(String(SKILL_ID), false))
