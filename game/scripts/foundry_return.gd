...
static func launch(from_combat: PlayerCombat, at: Vector3, mutation: Dictionary) -> void:
	var count := 0
	...
	var visual := CombatVisuals.projectile("frost", Color("b78a8a"))
	visual.scale = Vector3.ONE * 0.35
	mote.add_child(visual)
