...
static func build(root: Node3D, ground: Terrain) -> FrontierSites:
	if ground.world_profile() not in ["living_frontier_wave3","frontier_v11","frontier_v12","frontier_v13"]: return null
...
func _compose() -> void:
	...
	for habitat: Dictionary in terrain.map.frontier_hosts:
		...
		if terrain.world_profile() in ["frontier_v12","frontier_v13"] and String(habitat.influence)!="red": continue
...
func _scar(at: Vector3, influence: String, growth := false) -> void:
	...
	if terrain.world_profile() in ["frontier_v11","frontier_v12","frontier_v13"]:
...
func _laboratory(data: Dictionary) -> void:
	...
	var laboratory_open:=terrain._sim.campaign_policy()=="living_frontier_wave4"
	if laboratory_open:
		var body:=front.get_node("ExteriorBody")
		body.set_script(preload("res://scripts/laboratory_gate.gd"))
		body.set_meta("run_id","forge_capstone" if String(data.id).contains("central") else ("deep_forge" if String(data.id).contains("pairing") else "forge_tyrant"))
		body.set_meta("laboratory_label",String(data.label))
	...
	if laboratory_open: label.text="%s\n%s" % [String(data.label),"Human harness entrance" if String(data.id).contains("central") else ("Ordered pairing entrance" if String(data.id).contains("pairing") else "Containment entrance")]
