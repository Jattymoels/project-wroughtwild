	...
	for kind in kinds:
		...
		for centre in centres:
			...
			for entry in cover_by_biome[biome_index]:
				if reclaimed!=null and reclaimed.recovery.sample(centre.x,centre.z).x>float(reclaimed.recovery.settings.eligible_weight) and String(entry.kind).begins_with("scree"):continue
				if reclaimed!=null and reclaimed.highland!=null and reclaimed.highland.owns(centre,String(defs[biome_index].id),String(kind)): continue
