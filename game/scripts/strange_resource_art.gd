...
static func fixture_visual(kind: String) -> Node3D:
	...
	match kind:
		...
		"white_connection", "blue_delay", "green_junction", "red_heat_buffer":
			for entry: Dictionary in MACHINES.red_heat_parts if kind=="red_heat_buffer" else MACHINES.green_junction_parts if kind=="green_junction" else MACHINES.blue_delay_parts if kind=="blue_delay" else MACHINES.white_connection_parts:
