...
func _material_for(kind: String) -> Material:
	...
	if frontier_look != null:
		...
		if _world_profile in ["frontier_v10","frontier_v11","frontier_v12","frontier_v13"]:
			...
			for uniform: Dictionary in frontier_material.shader.get_shader_uniform_list():
				...
				if value!=null:parameters[uniform.name]=value
		...
		if _world_profile in ["frontier_v4", "frontier_v5", "frontier_v6","frontier_v7","frontier_v8","frontier_v9","frontier_v10","frontier_v11","frontier_v12","frontier_v13","living_frontier_wave1","living_frontier_wave3"] and _augmentation_texture != null:
		...
		if _world_profile in ["frontier_v10","frontier_v11","frontier_v12","frontier_v13"]: preload("res://land02b/kit.gd").bind_ground(frontier_material,map)
...
func build(sim: WroughtwildSim, seed_value: int, profile_id: String = "") -> void:
	...
	if weathered and _world_profile in ["frontier_v3", "frontier_v4", "frontier_v5", "frontier_v6","frontier_v7","frontier_v8","frontier_v9","frontier_v10","frontier_v11","frontier_v12","frontier_v13","living_frontier_wave1","living_frontier_wave3"]:
		...
		if _world_profile in ["frontier_v10","frontier_v11","frontier_v12","frontier_v13"]: atmosphere_look=preload("res://land02b/atmosphere.tres")
	...
	if map.is_empty():
		...
		return
	if _world_profile in ["frontier_v4", "frontier_v5", "frontier_v6","frontier_v7","frontier_v8","frontier_v9","frontier_v10","frontier_v11","frontier_v12","frontier_v13","living_frontier_wave1","living_frontier_wave3"]:
	...
	if weathered:
		...
		if _world_profile in ["frontier_v6","frontier_v7","frontier_v8","frontier_v9","frontier_v10","frontier_v11","frontier_v12","frontier_v13","living_frontier_wave1","living_frontier_wave3"]:
			...
			if _world_profile in ["frontier_v9","frontier_v10","frontier_v11","frontier_v12","frontier_v13"]: scarwater_cover=preload("res://land02/cover.gd").new(self)
			if _world_profile in ["frontier_v11","frontier_v12","frontier_v13"] and not map.get("dry_steppe",[]).is_empty(): steppe_cover=preload("res://land03/cover.gd").new(self)
			if _world_profile in ["frontier_v12","frontier_v13"]:
				preload("res://land04/kit.gd").prepare_resources()
				force_journeys=preload("res://land04/journeys.gd").new(self)
	if _world_profile in ["frontier_v3", "frontier_v4", "frontier_v5", "frontier_v6","frontier_v7","frontier_v8","frontier_v9","frontier_v10","frontier_v11","frontier_v12","frontier_v13","living_frontier_wave1","living_frontier_wave3"]:
...
func _build_chunk_phase(chunk_data: Dictionary,cell: float,phase: int,chunk: Node3D=null) -> Node3D:
	if phase==0:
		...
		var cover_began := Time.get_ticks_usec() if play03_trace != null else 0
		if int(chunk_data.get("cover_slice",0))==0:
			LakeWater.build_chunk(self,chunk,chunk_data)
		if play03_trace != null: play03_trace.note_arrival("cover_water",cover_began)
		cover_began = Time.get_ticks_usec() if play03_trace != null else 0
		GroundCover.build_for_chunk(chunk, chunk_data, map, cell, frontier_look,_world_profile in ["frontier_v3", "frontier_v4", "frontier_v5", "frontier_v6","frontier_v7","frontier_v8","frontier_v9","frontier_v10","frontier_v11","frontier_v12","frontier_v13","living_frontier_wave1","living_frontier_wave3"],reclaimed_cover)
		if play03_trace != null: play03_trace.note_arrival("cover_ground",cover_began)
		cover_began = Time.get_ticks_usec() if play03_trace != null else 0
		if weathered and int(chunk_data.get("cover_slice",0))==0:
			HabitatCover.build(chunk,chunk_data,map,cell,_world_profile in ["frontier_v3", "frontier_v4", "frontier_v5", "frontier_v6","frontier_v7","frontier_v8","frontier_v9","frontier_v10","frontier_v11","frontier_v12","frontier_v13","living_frontier_wave1","living_frontier_wave3"],wetland_cover)

		if play03_trace != null: play03_trace.note_arrival("cover_habitat",cover_began)
		cover_began = Time.get_ticks_usec() if play03_trace != null else 0
		if int(chunk_data.get("cover_slice",0))==0:
			if scarwater_cover!=null: scarwater_cover.build(chunk,chunk_data)
		if play03_trace != null: play03_trace.note_arrival("cover_scarwater",cover_began)
		cover_began = Time.get_ticks_usec() if play03_trace != null else 0
		if int(chunk_data.get("cover_slice",0))==0:
			if steppe_cover!=null: steppe_cover.build(chunk,chunk_data)
		if play03_trace != null: play03_trace.note_arrival("cover_steppe",cover_began)
		cover_began = Time.get_ticks_usec() if play03_trace != null else 0
		if int(chunk_data.get("cover_slice",0))==0:
			if force_journeys!=null: force_journeys.build(chunk,chunk_data)
		if play03_trace != null: play03_trace.note_arrival("cover_journeys",cover_began)
		...
		if _world_profile in ["frontier_v3", "frontier_v4", "frontier_v5", "frontier_v6","frontier_v7","frontier_v8","frontier_v9","frontier_v10","frontier_v11","frontier_v12","frontier_v13","living_frontier_wave1","living_frontier_wave3"]: StrangeSites.refresh_cover_chunk(self,chunk)
