...
func _prepare_restore(player: WroughtwildPlayer, data: Dictionary, restoring := true) -> Dictionary:
	...
	if data.has("world_seed") and (float(data.world_seed) < -2147483648.0 or float(data.world_seed) > 2147483647.0):
		...
		return {}
	if String(data.get("world_profile", "legacy_v1")) in ["frontier_v6","frontier_v7","frontier_v8","frontier_v9","frontier_v10","living_frontier_wave1","living_frontier_wave3"] and int(data.get("world_seed", 0)) < 0:
	...
	if profile not in ["legacy_v1","frontier_v2","frontier_v3","frontier_v4","frontier_v5","frontier_v6","frontier_v7","frontier_v8","frontier_v9","frontier_v10","living_frontier_wave1","living_frontier_wave3"]:
...
func read(path: String, player: WroughtwildPlayer) -> bool:
	...
	if parsed is Dictionary:
		...
		if parsed.has("schema_version") and _valid_integer(parsed.schema_version) and int(parsed.schema_version)>SCHEMA_VERSION:
			...
			return false
		if parsed.has("world_profile") and _valid_text(parsed.world_profile) and String(parsed.world_profile) not in ["legacy_v1","frontier_v2","frontier_v3","frontier_v4","frontier_v5","frontier_v6","frontier_v7","frontier_v8","frontier_v9","frontier_v10","living_frontier_wave1","living_frontier_wave3"]:
