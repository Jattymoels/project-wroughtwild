...
@export var world_profile: String = "frontier_v13"
...
func _ready() -> void:
	...
	if OS.get_cmdline_user_args().has("--living-frontier-wave4") or OS.get_cmdline_user_args().has("--living-frontier-wave5") or OS.get_cmdline_user_args().has("--living-frontier-wave6") or OS.get_cmdline_user_args().has("--living-frontier-wave7"):
		world_profile = "living_frontier_wave3"
...
func _build_world(seed_value: int) -> void:
	...
	if world_profile in ["frontier_v10","frontier_v11","frontier_v12","frontier_v13"]: preload("res://land02b/kit.gd").prepare()
	if world_profile in ["frontier_v12","frontier_v13"]: preload("res://land04/kit.gd").prepare_resources()
...
func _physics_process(delta: float) -> void:
	if terrain.map.is_empty():
		return
	_tick_day(delta)
	if world_profile in ["frontier_v11","frontier_v12","frontier_v13","living_frontier_wave1","living_frontier_wave3"] and not player.trial.active() and player.combat.life > 0 and not player.work_panel.is_open() and not player.inventory_panel.is_open() and not player.class_panel.is_open() and not player.foundry_panel.is_open() and not player.chest_panel.is_open() and not player.hud.help_visible():
