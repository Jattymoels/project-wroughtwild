...
extends RefCounted
## Production read-only material/mesh adapter.
static var meshes := {}
...
static func material_for(role: String) -> Material:
	...
	if stems.has(role):
		...
		mat.albedo_texture = R2Resources.resource(path+"_albedo.png")
		...
		mat.normal_texture = R2Resources.resource(path+"_normal.png")
		mat.roughness_texture = R2Resources.resource(path+"_orm.png")
