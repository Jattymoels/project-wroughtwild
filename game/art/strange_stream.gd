...
@export var resource_build_budget_ms := 2.0 # Stop adding resource scenes once this frame's creation work reaches the budget; one whole node always completes.
...
@export var terrain_chunks_per_frame := 1 # One complete terrain preparation phase or queued cosmetic leyline tile each frame during exploration.
