...
func _configure() -> void:
	...
	if definition.has("nymph"):
		...
		add_child(finished)
		finished.setup(_mesh, actor, definition.nymph)
		rig = finished.rig
		_previous = actor.global_position
		actor.attack_released.connect(released)
		return
	if definition.has("beetle"):
		finished = BeetlePresentation.new()
		finished.name = "BeetlePresentation"
		add_child(finished)
		finished.setup(_mesh, actor, definition.beetle)
		rig = finished.rig
		_previous = actor.global_position
		actor.attack_released.connect(released)
		return
	if definition.has("ram"):
		finished = RamPresentation.new()
		finished.name = "RamPresentation"
		add_child(finished)
		finished.setup(_mesh, actor, definition.ram)
		rig = finished.rig
		_previous = actor.global_position
		actor.attack_released.connect(released)
		return
	if definition.has("crane"):
		finished = CranePresentation.new()
		finished.name = "CranePresentation"
		add_child(finished)
		finished.setup(_mesh, actor, definition.crane)
		rig = finished.rig
		_previous = actor.global_position
		actor.attack_released.connect(released)
		return
	if definition.has("porcupine"):
		finished = PorcupinePresentation.new()
		finished.name = "PorcupinePresentation"
		add_child(finished)
		finished.setup(_mesh, actor, definition.porcupine)
		rig = finished.rig
		_previous = actor.global_position
		actor.attack_released.connect(released)
		return
