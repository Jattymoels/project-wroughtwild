param([switch]$Boss, [switch]$Central, [switch]$Native, [switch]$Visuals, [switch]$Ending)
...
if ($lf6Options.Ending) { Invoke-LFEngine 'central-ending' @('--headless','--fixed-fps','60','res://tests/central_trial.tscn','--','--lf6-ending') }
if ($lf6Options.Central) {
    Invoke-LFEngine 'central' @('--headless','--fixed-fps','60','res://tests/central_trial.tscn')
    foreach ($lfPhase in 'boundary','ending') { Invoke-LFEngine ("central-"+$lfPhase) @('--headless','--fixed-fps','60','res://tests/central_trial.tscn','--',("--lf6-"+$lfPhase)) }
}
