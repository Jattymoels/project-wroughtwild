param(
 [ValidateSet('after','lifecycle','restore','boss_display')][string]$Job='after',
...
$rfOutput="$rfRoot/build/play03-group"
...
try {
 ...
 if($Job -eq 'import') {
  ...
  $arguments+= $(if($Job -eq 'boss_display'){'res://tests/play03_group_boss_display.tscn'}elseif($Job -eq 'restore'){'res://tests/play03_group_restore.tscn'}elseif($Job -eq 'lifecycle'){'res://tests/play03_group_lifecycle.tscn'}else{'res://tests/play03_group.tscn'})
  if($Job -in @('after','boss_display')) {
