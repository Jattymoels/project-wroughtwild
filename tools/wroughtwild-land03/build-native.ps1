...
if($LASTEXITCODE -ne 0){throw 'LAND03 native configure failed'}
& $rfCmake --build "$rfOut/native" -j 6 *> "$rfOut/native/build.log"
if($LASTEXITCODE -ne 0){Get-Content "$rfOut/native/build.log" -Tail 35;throw 'LAND03 native build failed'}
...
$rfFiles+=@('sim/src/worldgen_frontier_v7.inc','tools/wroughtwild-land03/CMakeLists.txt','tools/wroughtwild-land03/build-native.ps1')
...
Write-Output "LAND03_BUILT $rfDll"
