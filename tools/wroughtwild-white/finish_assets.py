...
cfg=json.loads(Path(__file__).with_name('white.json').read_text(encoding='utf-8'));sc=cfg['scar']
...
host=bpy.data.objects['White inclusion - intact normalized source']
...
original.name='SOURCE - untouched normalized White inclusion';original.hide_set(True);original.hide_render=True
...
host.name='White inclusion - ready worked or spent host';mesh=host.data
...
for name,role in [('white-base.png','baseColorTexture'),('white-orm.png','metallicRoughnessTexture')]:
...
mat=mesh.materials[0].copy();mesh.materials[0]=mat;mat.name='WHITE_MINERAL - attached scar'
...
size=sc['mask_size'];mask=bpy.data.images.new('White scar - linear core damage travel',width=size,height=size,alpha=True,float_buffer=True);mask.colorspace_settings.name='Non-Color'
...
mask.pixels.foreach_set(sample(positions).ravel());mask.update();mask.filepath_raw=str((out/'white-scar.png').resolve());mask.file_format='PNG';mask.save();mask.pack()
...
normal=bpy.data.images.new('White fracture tangent normal',width=size,height=size,alpha=False)
normal.colorspace_settings.name='Non-Color'
normal_target=nodes.new('ShaderNodeTexImage');normal_target.image=normal;nodes.active=normal_target
bpy.ops.object.bake(type='NORMAL')
normal.filepath_raw=str((out/'white-normal.png').resolve());normal.file_format='PNG';normal.save();normal.pack()
def triangles(o):
    o.data.calc_loop_triangles();return len(o.data.loop_triangles)
def export(objects,name):
    bpy.ops.object.select_all(action='DESELECT')
    if name.startswith('white-source-') or name.startswith('white-fragment-'):
...
for target_count,name in zip(cfg['lod_triangles'],['near','mid','far']):
    lod=host if name=='near' else copy(host,'White inclusion - '+name)
    if name!='near':
    ...
    export([lod],'white-source-'+name+'.glb');report['lods'][name]=triangles(lod)
...
def material(name,color,metallic=0.,rough=.85):
    ...
    return m
salt_cut=material('WHITE_CUT - fresh ceramic salt face',(.16,.037,.018),.05,.87)
def bake_fragment(piece,index):
    ...
    fm=material('WHITE_FRAGMENT - '+str(index+1),(.3,.28,.23));piece.data.materials.clear();piece.data.materials.append(fm)
    ...
    for kind in ['base','orm','scar']:
        original_image=bpy.data.images.load(str((out/('white-'+kind+'.png')).resolve()),check_existing=True)
        ...
        im=bpy.data.images.new('White fragment %d %s'%(index+1,kind),width=resolution,height=resolution,alpha=False)
    ...
    normal=bpy.data.images.new('White fragment own tangent normal',width=resolution,height=resolution,alpha=False);normal.colorspace_settings.name='Non-Color';normal_node=fn.new('ShaderNodeTexImage');normal_node.image=normal;fn.active=normal_node;bpy.ops.object.bake(type='NORMAL')
...
for index,fraction in enumerate([.2,.5,.78]):
    center=np.array(paths[0]['projected'][int(fraction*(len(paths[0]['projected'])-1))])-np.array([0,0,.045])
    center=center.tolist();size=(.27,.25,.23)
    piece=copy(host,'White Mineral - cut fragment '+str(index+1));piece.data.materials.append(salt_cut)
    ...
    export([piece],'white-fragment-'+str(index+1)+'.glb')
...
# Supported timber signal post; the top terminal reaches native height 1.18 m.
cube('Broad timber foot',(0,0,.065),(.62,.52,.13),timber,.018)
cube('Timber mast',(0,0,.42),(.19,.20,.66),timber,.012)
for x in [-.19,.19]:
    brace=cube('Splayed shoulder brace',(x,0,.27),(.075,.16,.39),iron,.009)
    brace.rotation_euler.y=-math.copysign(.42,x)
for z in [.15,.51]:
    cube('Forged mast collar',(0,0,z),(.245,.25,.065),iron,.008)
    bolt((0,-.137,z))
cube('Mineral saddle',(0,0,.71),(.53,.41,.09),iron,.012)
for x in [-.245,.245]:
    cube('Side compression jaw',(x,0,.87),(.055,.16,.35),edge,.01)
    jaw=cube('Folded upper jaw',(x*.73,0,1.018),(.19,.15,.045),edge,.008)
    jaw.rotation_euler.y=math.copysign(.33,x)
    bolt((x,-.095,.9))
cube('Signal terminal neck',(0,0,1.095),(.065,.07,.12),iron,.007)
cube('Signal terminal crown',(0,0,1.164),(.15,.14,.032),edge,.006)
# The same mineral remains visibly captured between the deliberately made jaws.
core=copy(host,'WHITE_CORE - fitted inclusion');core.scale=(.31,.29,.34);core.location=(0,0,.752)
parts.append(core)
...
export(housing+[core],'white-post-near.glb')
...
for name,target_count in zip(['mid','far'],cfg['lod_triangles'][1:]):
    c=copy(core,'WHITE_CORE - '+name);bpy.context.view_layer.objects.active=c
    ...
    export(housing+[c],'white-post-'+name+'.glb');report['post'][name+'_core_triangles']=triangles(c)
    c.hide_render=True;c.hide_set(True)
for im in bpy.data.images:
    if im.has_data and im.source=='FILE':im.pack()
scene['study']='ART-04 White isolated handoff; native rules/body/ownership unchanged'
(out/'asset-report.json').write_text(json.dumps(report,indent=2),encoding='utf-8')
bpy.ops.wm.save_as_mainfile(filepath=str((out/'white-workshop.blend').resolve()),compress=True)
assert hashlib.sha256(raw.read_bytes()).hexdigest()==cfg['source_sha256']
