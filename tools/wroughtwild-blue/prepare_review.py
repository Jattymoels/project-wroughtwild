"""Prepare Blue review using the recorded frozen ART-04 native binary.
...
checkpoint=repo/'build/lf2/published-a/lf2-pending.json'
shutil.copy2(checkpoint,review/'paid-checkpoint.json')
for p in recipe.iterdir():
    if p.suffix in ['.gd','.gdshader','.godot','.tscn'] or p.name=='blue.json':shutil.copy2(p,review/p.name)
if assets.is_dir():
...
print('BLUE_REVIEW_PREPARED',review)
