...
cfg=json.loads(Path(__file__).with_name('blue.json').read_text(encoding='utf-8'));sc=cfg['scar']
...
host=bpy.data.objects['Blue inclusion - intact normalized source']
...
original.name='SOURCE - untouched normalized Blue inclusion';original.hide_set(True);original.hide_render=True
...
host.name='Blue inclusion - ready worked or spent host';mesh=host.data
...
for name,role in [('blue-base.png','baseColorTexture'),('blue-orm.png','metallicRoughnessTexture')]:
...
mat=mesh.materials[0].copy();mesh.materials[0]=mat;mat.name='BLUE_MINERAL - attached scar'
...
size=sc['mask_size'];mask=bpy.data.images.new('Blue scar - linear core damage travel',width=size,height=size,alpha=True,float_buffer=True);mask.colorspace_settings.name='Non-Color'
...
mask.pixels.foreach_set(sample(positions).ravel());mask.update();mask.filepath_raw=str((out/'blue-scar.png').resolve());mask.file_format='PNG';mask.save();mask.pack()
...
normal=bpy.data.images.new('Blue fracture tangent normal',width=size,height=size,alpha=False)
normal.colorspace_settings.name='Non-Color'
normal_target=nodes.new('ShaderNodeTexImage');normal_target.image=normal;nodes.active=normal_target
bpy.ops.object.bake(type='NORMAL')
normal.filepath_raw=str((out/'blue-normal.png').resolve());normal.file_format='PNG';normal.save();normal.pack()
def triangles(o):
    o.data.calc_loop_triangles();return len(o.data.loop_triangles)
def export(objects,name):
    bpy.ops.object.select_all(action='DESELECT')
    if name.startswith('blue-source-') or name.startswith('blue-fragment-'):
...
for target_count,name in zip(cfg['lod_triangles'],['near','mid','far']):
    lod=host if name=='near' else copy(host,'Blue inclusion - '+name)
    if name!='near':
    ...
    export([lod],'blue-source-'+name+'.glb');report['lods'][name]=triangles(lod)
...
def material(name,color,metallic=0.,rough=.85):
    ...
    return m
salt_cut=material('BLUE_CUT - exposed slate face',(.028,.045,.07),.05,.87)
def bake_fragment(piece,index):
    ...
    fm=material('BLUE_FRAGMENT - '+str(index+1),(.3,.28,.23));piece.data.materials.clear();piece.data.materials.append(fm)
    ...
    for kind in ['base','orm','scar']:
        original_image=bpy.data.images.load(str((out/('blue-'+kind+'.png')).resolve()),check_existing=True)
        ...
        im=bpy.data.images.new('Blue fragment %d %s'%(index+1,kind),width=resolution,height=resolution,alpha=False)
    ...
    normal=bpy.data.images.new('Blue fragment own tangent normal',width=resolution,height=resolution,alpha=False);normal.colorspace_settings.name='Non-Color';normal_node=fn.new('ShaderNodeTexImage');normal_node.image=normal;fn.active=normal_node;bpy.ops.object.bake(type='NORMAL')
...
for index,fraction in enumerate([.2,.5,.78]):
    center=np.array(paths[0]['projected'][int(fraction*(len(paths[0]['projected'])-1))])-np.array([0,0,.022])
    center=center.tolist();size=(.29,.23,.15)
    piece=copy(host,'Blue Mineral - cut fragment '+str(index+1));piece.data.materials.append(salt_cut)
    ...
    assert all(e.is_manifold for e in bm.edges),('Unclosed slate fragment',index)
    assert abs(bm.calc_volume())>.0001,('Zero-volume slate fragment',index)
    ...
    export([piece],'blue-fragment-'+str(index+1)+'.glb')
...
# Precisely fitting player-built post: hammered frame, ceramic chamber,
...
# Twin supports and nested retaining jaws hold the same slate vertically.
# This is a made delay fitting, with the unchanged single terminal at 1.18 m.
cube('Heavy timber foot',(0,0,.055),(.62,.52,.11),timber,.015)
for x in [-.23,.23]:
    cube('Paired timber support',(x,.045,.52),(.09,.15,.91),timber,.009)
    cube('Forged foot strap',(x,0,.13),(.13,.29,.055),iron,.007)
    bolt((x,-.10,.18))
cube('Lower mineral cradle',(0,0,.44),(.53,.36,.065),iron,.010)
for z,width in [(.54,.51),(.76,.48),(.96,.42)]:
    for side in [-1,1]:
        cube('Nested retaining lip',(side*width/2,0,z),(.052,.28,.07),edge,.008)
        lip=cube('Inward folded retainer',(side*(width/2-.035),-.135,z+.016),(.13,.038,.042),edge,.006)
        lip.rotation_euler.y=side*.18
        bolt((side*width/2,-.15,z))
cube('Top bridge',(0,.035,1.038),(.55,.21,.065),iron,.012)
cube('Signal terminal neck',(0,0,1.105),(.065,.07,.11),iron,.007)
cube('Signal terminal crown',(0,0,1.164),(.15,.14,.032),edge,.006)
core=copy(host,'BLUE_CORE - fitted inclusion');core.scale=(.32,.36,.32)
core.rotation_mode='XYZ';core.rotation_euler.x=math.pi/2;core.location=(0,.11,.694)
parts.append(core)
...
export(housing+[core],'blue-post-near.glb')
report['post']={'housing_triangles':triangles(housing[0]),'core_triangles':triangles(core),'body_godot_xyz':cfg['post_body'],'port_height':cfg['port_height']}
# Smaller core meshes retain the authored mask and housing body.
for name,target_count in zip(['mid','far'],cfg['lod_triangles'][1:]):
    c=copy(core,'BLUE_CORE - '+name);bpy.context.view_layer.objects.active=c
    d=c.modifiers.new('Core distance reduction','DECIMATE');d.ratio=target_count/triangles(core);bpy.ops.object.modifier_apply(modifier=d.name)
    export(housing+[c],'blue-post-'+name+'.glb');report['post'][name+'_core_triangles']=triangles(c)
    c.hide_render=True;c.hide_set(True)
for im in bpy.data.images:
    if im.has_data and im.source=='FILE':im.pack()
scene['study']='ART-04 Blue isolated handoff; native rules/body/ownership unchanged'
(out/'asset-report.json').write_text(json.dumps(report,indent=2),encoding='utf-8')
bpy.ops.wm.save_as_mainfile(filepath=str((out/'blue-workshop.blend').resolve()),compress=True)
assert hashlib.sha256(raw.read_bytes()).hexdigest()==cfg['source_sha256']
