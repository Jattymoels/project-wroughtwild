...
for kind,limit in [('source',np.array([1.5,1.5,1.1])),('post',np.array([.65,.55,1.18]))]:
    for lod in ['near','mid','far']:
        path=assets/f'blue-{kind}-{lod}.glb'
...
for i in range(1,4):
    path=assets/f'blue-fragment-{i}.glb'
    bpy.ops.wm.read_factory_settings(use_empty=True);bpy.ops.import_scene.gltf(filepath=str(path))
    ...
    bm.free()
bpy.ops.wm.open_mainfile(filepath=str(assets/'blue-workshop.blend'))
check('SOURCE - untouched normalized Blue inclusion' in bpy.data.objects,'dense untouched source retained')
check('SOURCE - editable procedural housing' in bpy.data.objects,'editable material authoring retained')
core_for_fit=bpy.data.objects['BLUE_CORE - fitted inclusion']
bpy.context.view_layer.update()
core_low=min((core_for_fit.matrix_world@v.co).z for v in core_for_fit.data.vertices)
check(-.004<=core_low-(.44+.065/2)<=.002,'captured mineral seats on its physical cradle without floating')
textures=[(name,2048) for name in ['blue-base.png','blue-orm.png','blue-normal.png','blue-scar.png','post-base.png','post-orm.png','post-normal.png']]
...
host=bpy.data.objects['Blue inclusion - ready worked or spent host'];host.location.x=-1.25
...
core=bpy.data.objects['BLUE_CORE - fitted inclusion'];core['export_location']=list(core.location);core.location.x+=1.1
shown=[host,housing,core]
for i in range(3):
    o=bpy.data.objects['Blue Mineral - cut fragment '+str(i+1)];o.location=(i*.35-.35,-.8,0);shown.append(o)
...
bpy.ops.wm.save_as_mainfile(filepath=str(out/'blue-workshop.blend'),compress=True)
scene.render.filepath=str(out/'editable-preview.png');bpy.ops.render.render(write_still=True)
bpy.ops.wm.open_mainfile(filepath=str(out/'blue-workshop.blend'))
for im in bpy.data.images:
...
report={'checks':checks,'failures':[],'runtime_assets':records,'blender':bpy.app.version_string,'packed_blend_sha256':hashlib.sha256((out/'blue-workshop.blend').read_bytes()).hexdigest()}
