...
$rfState="$rfRoot/build/rf06/$rfMode"
$rfPrior=@{}
...
try {
 ...
 if(-not $Fresh) {
  ...
  if(-not (Test-Path -LiteralPath $rfSave)) {
   ...
   if($Fen) {
    $rfFixture=Get-Content -LiteralPath "$rfRoot/build/rf06/private-world.json" -Raw | ConvertFrom-Json
    $rfPoint=(Get-Content -LiteralPath "$rfRoot/build/rf06/expected.json" -Raw | ConvertFrom-Json).fen
   ...
   } else {Copy-Item -LiteralPath "$rfRoot/build/rf06/private-world.json" -Destination $rfSave}
  }
