...
arm=bpy.data.objects['Rimejaw - study rig']
mesh=bpy.data.objects['Rimejaw - near candidate']
mesh.hide_set(False);arm.hide_set(False)
lods={name:bpy.data.objects[object_name] for name,object_name in [('mid','Rimejaw - mid candidate'),('far','Rimejaw - far candidate')]}
...
for clip,duration in rig['clips_seconds'].items():
    ...
    for sample in range(samples):
        ...
        for name,value in floors.items():
            ...
            assert value<=(.058 if clip=='walk' else .003),(clip,sample,name,'hovering',value)
        lod_minima={}
        ...
        for obj in [o for o in bpy.context.scene.objects if o.name.startswith('Rimejaw - fitted')]:
        ...
        extension=float(np.max(np.linalg.norm(coordinates[edges[:,1]]-coordinates[edges[:,0]],axis=1)-rest_lengths))
        assert extension<.08,('Visible stretched mesh edge',clip,sample,extension)
        records.append({'max_edge_extension_metres':extension,'clip':clip,'seconds':frame/100,'lowest_surface_metres':float(coordinates[:,2].min()),'lod_lowest_surface_metres':lod_minima,'hoof_sole_y_metres':floors})
