...
var status := false
var orbit := 0.0
var distance_metres := 6.8
var config: Dictionary
var output := "res://evidence"
const POSES := ["idle", "walk", "graze", "turn"]
const MODES := ["Light off", "Steady", "Breathing", "Travelling"]

func _ready() -> void:
	config = JSON.parse_string(FileAccess.get_file_as_string("res://stag.json"))
	minimum = config.scar.minimum_light
	period = config.scar.period_seconds
	material = ShaderMaterial.new()
	material.shader = load("res://fauna_scar.gdshader")
	...
	sunlight.rotation_degrees = Vector3(-42, 145, 0)
...
func _build_actor_set(count: int) -> void:
	var names := ["base.png", "orm.png", "scar-mask.png", "normal.png"]
	...
	actors.clear()
	players.clear()
	var scene: PackedScene = load("res://stag-" + lod + ".glb")
	for index in count:
		var actor: Node3D = scene.instantiate()
		add_child(actor)
		actors.append(actor)
		if count > 1:
			actor.position = Vector3((index % 6 - 2.5) * 3.2, 0, (index / 6 - 1.5) * 3.4)
		for node in actor.find_children("*", "MeshInstance3D", true, false):
			if "candidate" in node.name:
...
func _unhandled_key_input(event: InputEvent) -> void:
	...
	match event.keycode:
		...
		KEY_B: minimum = config.scar.minimum_light if minimum == 0.5 else 0.5
				...
				for bone in skeleton.get_bone_count():
					...
					if skeleton.get_bone_name(bone).ends_with("_hoof"):
						...
						if clip != "walk":
							...
							assert(hoof_y >= 0.099 and hoof_y <= 0.191)
		results.append({"lod": detail, "skin_vertices": weights.size()/4, "bones": 17, "clips_checked": POSES, "aabb": str(mesh.get_aabb())})
	_build_actor_set(2)
	var first: MeshInstance3D = _host(actors[0])
	var second: MeshInstance3D = _host(actors[1])
...
func _capture_suite() -> void:
	...
	shade = false
	for clip in POSES:
		_set_pose(clip, players[0].get_animation(clip).length * (0.25 if clip == "turn" else 0.5))
		await _capture("pose-" + clip)
	_set_pose("graze", 3.0)
	for side in [-1.0, 1.0]:
		var target := Vector3(0, 0.95, -1.10)
		camera.position = target + Vector3(side*3.0, 0.65, -3.2)
		camera.look_at(target)
		await _capture("graze-close-" + str(side))
	_camera_view()
	_set_pose("turn", players[0].get_animation("turn").length * 0.75)
	await _capture("turn-opposite")
	status = true
	...
	for frame in 144:
		clock = frame / 12.0
		_set_pose("graze", fmod(clock, 6.0))
		await _capture("graze-frame-%03d" % frame)
...
func _benchmark_suite() -> void:
	...
	for count in [1, 24]:
		for detail in ["near", "mid", "far"]:
			...
			distance_metres = 6.8 if count == 1 else 17.0
			_camera_view()
