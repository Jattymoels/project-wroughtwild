...
const POSES := ["idle", "walk", "root", "turn", "jaw", "windup", "release"]
const MODES := ["Light off", "Steady", "Breathing", "Travelling"]

func _ready() -> void:
	config = JSON.parse_string(FileAccess.get_file_as_string("res://wolf.json"))
...
func _build_actor_set(count: int) -> void:
	...
	actors.clear()
	players.clear()
	var scene: PackedScene = load("res://wolf-" + lod + ".glb")
	for index in count:
				...
				for bone in skeleton.get_bone_count():
					...
					if skeleton.get_bone_name(bone).ends_with("_hoof"):
						...
						if clip != "walk":
							...
							assert(hoof_y >= 0.099 and hoof_y <= 0.156)
...
func _capture_suite() -> void:
	...
	_set_pose("jaw", 1.5)
	for side in [-1.0, 1.0]:
		var target := Vector3(-0.035, 0.46, -0.66)
		camera.position = target + Vector3(side*0.62, 0.18, -0.85)
		camera.look_at(target)
		await _capture("jaw-close-" + str(side))
	_camera_view()
	...
	for frame in 144:
		clock = frame / 12.0
		_set_pose("jaw", fmod(clock, 3.0))
		await _capture("jaw-frame-%03d" % frame)
