"""Blender: bake attached fracture/core/travel data on the finished wolf host.
...
host=bpy.data.objects['Rimejaw - host'];mesh=host.data
...
material.name='Rimejaw - scar host'
...
scene['study']='ART-03 isolated wolf - not adopted game art'
bpy.ops.wm.save_as_mainfile(filepath=str(output/'wolf-surface.blend'),compress=True)
assert hashlib.sha256(raw.read_bytes()).hexdigest()==cfg['source_sha256']
print('WOLF_SURFACE_OK '+str(output),flush=True)
