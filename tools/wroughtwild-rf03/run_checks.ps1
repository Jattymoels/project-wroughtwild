param(
 [ValidateSet('import','identity','build','walk','continue','v6','diagnose')][string]$Job='build',
...
$env:WROUGHTWILD_RF03_OUTPUT=$rfOutput
$mutex=$null
$held=$false
$madeOverride=$false
$process=$null
...
try {
 ...
 if($Job -eq 'import') {
  ...
  $arguments+="res://tests/rf03/$Job.tscn"
  if($Job -eq 'walk') {
