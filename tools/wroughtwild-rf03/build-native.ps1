...
if($LASTEXITCODE -ne 0){throw 'RF03 native configure failed'}
& $rfCmake --build "$rfOut/native" -j 6 *> "$rfOut/native/build.log"
if($LASTEXITCODE -ne 0){Get-Content "$rfOut/native/build.log" -Tail 35;throw 'RF03 native build failed'}
...
$rfFiles=@(& git -C $rfRoot ls-files sim game/extensions/wroughtwild_sim)
$rfFiles+=@('sim/src/worldgen_frontier_v7.inc','tools/wroughtwild-rf03/CMakeLists.txt','tools/wroughtwild-rf03/build-native.ps1')
