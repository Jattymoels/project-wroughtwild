...
$g2Build=[IO.Path]::GetFullPath((Join-Path $PSScriptRoot '../../../build/art07/g2'))
function Assert-G2Path([string]$Value) {
 $resolved=[IO.Path]::GetFullPath($Value)
 if (-not $resolved.StartsWith($g2Build+[IO.Path]::DirectorySeparatorChar,[StringComparison]::OrdinalIgnoreCase)) { throw ('Outside G2 output: '+$resolved) }
 return $resolved
}
...
foreach($g2Job in $g2Jobs) {
 ...
 try {
  ...
  while(-not $g2Process.WaitForExit(1000)) {
   ...
   if($g2Failure){Stop-Process -Id $g2Process.Id -ErrorAction SilentlyContinue;$g2Process.WaitForExit();break}
  }
