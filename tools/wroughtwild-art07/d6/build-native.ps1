param([string]$Revision='f00d4b76274ef2a73c3b6f4a275d2bd3ca0f958d', [Parameter(Mandatory)][string]$Output, [Parameter(Mandatory)][string]$Depot)
$ErrorActionPreference='Stop'
$artRepo=[IO.Path]::GetFullPath($Depot)
$artOutput=[IO.Path]::GetFullPath($Output)
$d6Worker=[IO.Path]::GetFullPath((Join-Path $PSScriptRoot '../../..'))
if (-not $artOutput.StartsWith((Join-Path $d6Worker 'build/art07/d6')+[IO.Path]::DirectorySeparatorChar)) { throw 'Output must remain in the D6 build tree.' }
...
project(art07_d6_native LANGUAGES CXX)
...
Write-Output 'D6_NATIVE_BUILD_OK'
