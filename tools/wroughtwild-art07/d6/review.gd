...
func _ready() -> void:
	...
	var model: Node3D = load("res://d6_proxies.glb").instantiate()
	add_child(model)
	var materials := {}
	for family: Dictionary in families:
		for role: String in ["body", "seam"]:
			var key: String = "d6_" + str(family.id) + "_" + role
			var material := StandardMaterial3D.new()
			material.resource_name = key
			var texture_key: String = "d6_" + str(family.id)
			material.albedo_texture = load("res://textures/" + texture_key + "_albedo.png")
			material.normal_enabled = true
			material.normal_texture = load("res://textures/" + texture_key + "_normal.png")
			material.roughness_texture = load("res://textures/" + texture_key + "_orm.png")
			material.roughness_texture_channel = BaseMaterial3D.TEXTURE_CHANNEL_GREEN
			material.texture_filter = BaseMaterial3D.TEXTURE_FILTER_LINEAR_WITH_MIPMAPS_ANISOTROPIC
			material.texture_repeat = true
			material.metallic = float(family.seam_metallic) if role == "seam" else 1.0
			...
			if role == "seam":
				material.albedo_color = Color(family.seam_tint[0], family.seam_tint[1], family.seam_tint[2])
	...
	assert(mesh_count == 10 and triangles == int(audit.triangles), "Imported proxy topology differs")
	...
	sky_material.sky_top_color = Color(0.60,0.60,0.60)
	sky_material.sky_horizon_color = Color(0.85,0.85,0.85)
	sky_material.ground_bottom_color = Color(0.50,0.50,0.50)
	sky_material.ground_horizon_color = Color(0.70,0.70,0.70)
	sky_material.sun_angle_max = 1.0
	sky.sky_material = sky_material
	environment.sky = sky
	environment.background_energy_multiplier = 1.3
	...
	camera.size = 6.5
	add_child(camera)
	view_at(Vector3(2.5,5,15), Vector3(0,1.2,0), 5.8)
	...
	caption.text = "ART-07D6 / " + renderer + " / 1–4: family   0: overview   Esc: exit"
	var args := OS.get_cmdline_user_args()
	for i in 12: await get_tree().process_frame
	print("D6_IMPORT_OK meshes=%d triangles=%d renderer=%s device=%s" % [mesh_count,triangles,renderer,RenderingServer.get_video_adapter_name()])
	if args.has("--capture"):
		await capture("overview", "NEUTRAL / IRON · BRONZE · STEEL · SILVER / static material proxies")
		for i in families.size():
			family_view(i)
			await capture(str(families[i].id), str(families[i].name).to_upper() + " / actual joints and finish")
		view_at(Vector3(2.5,5,15),Vector3(0,1.2,0),5.8)
		...
		for distance: float in [4.0,10.0,24.0]:
			view_at(Vector3(0,2.8,distance),Vector3(0,1.15,0),5.8)
			await capture("distance-"+str(int(distance)), "SHADE / camera " + str(int(distance)) + " m / same geometry, mipmapped maps")
		camera.projection = Camera3D.PROJECTION_ORTHOGONAL
		view_at(Vector3(2.5,5,15),Vector3(0,1.2,0),5.8)
		...
		for i in 96:
			var x := lerpf(-3.45,3.45,float(i)/95.0)
			view_at(Vector3(x+1.05,2.7,4.0),Vector3(x,1.2,0),3.1)
			caption.text = "ART-07D6 / " + renderer + " / camera inspection · static, non-emissive materials"
			await RenderingServer.frame_post_draw
			get_viewport().get_texture().get_image().save_png("res://evidence/"+renderer+"-motion/%04d.png" % i)
		get_tree().quit()
	elif args.has("--benchmark"):
		caption.text = "ART-07D6 / fixed view / benchmark without image capture"
		...
		print("D6_BENCHMARK ",JSON.stringify(result))
		get_tree().quit()
...
func capture(name_id: String, title: String) -> void:
	caption.text = "ART-07D6 / " + renderer + " / " + title
...
func _unhandled_key_input(event: InputEvent) -> void:
	...
	if key.keycode == KEY_0: view_at(Vector3(2.5,5,15),Vector3(0,1.2,0),5.8)
	if key.keycode >= KEY_1 and key.keycode <= KEY_4: family_view(key.keycode-KEY_1)
