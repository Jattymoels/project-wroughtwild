...
if(-not $artOut.StartsWith($artRoot+[IO.Path]::DirectorySeparatorChar) -or (Test-Path $artOut)){throw 'Fresh B2 output required'}
New-Item -ItemType Directory -Path $artOut | Out-Null
$artInput=(Resolve-Path (Join-Path $PSScriptRoot '../../../docs/art/leyline-studies/2026-09-09/art07/b2/shrub-input-v01.png')).Path
