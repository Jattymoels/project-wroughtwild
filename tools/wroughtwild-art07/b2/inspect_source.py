"""Fresh actual shrub mesh inspection; six material and six clay directions."""
...
base=Vector(((lo.x+hi.x)/2,(lo.y+hi.y)/2,lo.z));factor=1.35/(hi.z-lo.z)
for o in obs:
...
report={'raw_sha256':__import__('hashlib').sha256(source.read_bytes()).hexdigest(),'raw_bounds':[list(lo),list(hi)],'scale':factor,'subtract_raw_base':list(base),'height_m':1.35,'objects':len(obs)}
...
cam=bpy.data.objects.new('Camera',bpy.data.cameras.new('Camera'));scene.collection.objects.link(cam);scene.camera=cam;cam.data.type='ORTHO';cam.data.ortho_scale=1.9
...
for name,at in [('front',(0,-4,.7)),('back',(0,4,.7)),('side',(4,0,.7)),('threequarter',(3,-4,2)),('top',(0,-.01,5)),('underside',(3,-4,-2))]:
    cam.location=at;cam.rotation_euler=(Vector((0,0,.65))-cam.location).to_track_quat('-Z','Y').to_euler()
...
bpy.ops.wm.save_as_mainfile(filepath=str(out/'shrub-source.blend'),compress=True)
(out/'inspection.json').write_text(json.dumps(report,indent=2));print('B2_INSPECTION_OK')
