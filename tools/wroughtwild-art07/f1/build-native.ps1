param([string]$Revision='bbcb3a7dfd235e8f803141ccb57c38e03d6c1708', [Parameter(Mandatory)][string]$Output, [Parameter(Mandatory)][string]$Depot)
$ErrorActionPreference='Stop'
$artRepo=[IO.Path]::GetFullPath($Depot)
$artOutput=[IO.Path]::GetFullPath($Output)
$f1Worker=[IO.Path]::GetFullPath((Join-Path $PSScriptRoot '../../..'))
if (-not $artOutput.StartsWith((Join-Path $f1Worker 'build/art07/f1')+[IO.Path]::DirectorySeparatorChar)) { throw 'Output must remain in the F1 build tree.' }
...
project(art07_f1_native LANGUAGES CXX)
...
Write-Output 'F1_NATIVE_BUILD_OK'
