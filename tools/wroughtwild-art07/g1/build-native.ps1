param([string]$Revision='6bb2e044dcd0bf1788896aa2c19cdf56fee93522', [Parameter(Mandatory)][string]$Output, [Parameter(Mandatory)][string]$Depot)
$ErrorActionPreference='Stop'
$artRepo=[IO.Path]::GetFullPath($Depot)
$artOutput=[IO.Path]::GetFullPath($Output)
$g1Worker=[IO.Path]::GetFullPath((Join-Path $PSScriptRoot '../../..'))
if (-not $artOutput.StartsWith((Join-Path $g1Worker 'build/art07/g1')+[IO.Path]::DirectorySeparatorChar)) { throw 'Output must remain in the G1 build tree.' }
...
project(art07_g1_native LANGUAGES CXX)
...
Write-Output 'G1_NATIVE_BUILD_OK'
