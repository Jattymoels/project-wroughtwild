...
static func mount_source(owner_node: LeylineSource) -> void:
	...
	for i in 3:
		var fragment: Node3D=load("res://f5/"+root.colour+"/"+root.colour+"-fragment-%d.glb"%(i+1)).instantiate()
...
func scar(prefix: String="") -> ShaderMaterial:
	...
	mat.shader=load("res://f5/"+colour+"/"+colour+"_scar.gdshader")
	for pair in [["base_texture","base"],["orm_texture","orm"],["scar_texture","scar"],["normal_texture","normal"]]:
		mat.set_shader_parameter(pair[0],load("res://f5/"+colour+"/"+prefix+"-"+pair[1]+".png"))
...
func add_model(role: String) -> void:
	...
	var model: Node3D=load("res://f5/"+colour+"/"+colour+"-"+role+"-"+String(integration.colour_lod)+".glb").instantiate()
	add_child(model)
	mineral=scar()
