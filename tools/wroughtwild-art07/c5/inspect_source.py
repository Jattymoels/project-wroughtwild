"""Read-only inspection of approved ART-02 master; output only to fresh B3 folder."""
...
scene.world=bpy.data.worlds.new('B3 neutral');scene.world.use_nodes=True;scene.world.node_tree.nodes['Background'].inputs[0].default_value=(.3,.34,.4,1)
light=bpy.data.objects.new('B3 sun',bpy.data.lights.new('B3 sun','SUN'));scene.collection.objects.link(light);light.rotation_euler=(.5,-.6,-.5);light.data.energy=2
cam=bpy.data.objects.new('B3 camera',bpy.data.cameras.new('B3 camera'));scene.collection.objects.link(cam);scene.camera=cam;cam.data.type='ORTHO';cam.data.ortho_scale=3.7
