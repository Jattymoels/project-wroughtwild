...
$artRoot=[IO.Path]::GetFullPath((Join-Path $PSScriptRoot '../../../build/art07/c1'))
$artLog=[IO.Path]::GetFullPath($Log)
if(-not $artLog.StartsWith($artRoot+[IO.Path]::DirectorySeparatorChar)){throw 'Log must be in C1 output.'}
...
try {
 ...
 Write-Output "C1 own PID $($artProc.Id)"
 while(-not $artProc.WaitForExit(1000)){
  if(Select-String -Path $artLog,"$artLog.stderr" -Pattern 'SCRIPT ERROR|SHADER ERROR|Parse Error|Traceback' -Quiet){if(Get-Process -Id $artProc.Id -ErrorAction SilentlyContinue){Stop-Process -Id $artProc.Id}; $artProc.WaitForExit();throw "This C1 PID reported script/shader failure: $artLog"}
 }
 $artProc.Refresh()
 @{program=$Program;arguments=$Arguments;pid=$artProc.Id;exit=$artProc.ExitCode;started_utc=$artStart.ToUniversalTime().ToString("o");finished_utc=(Get-Date).ToUniversalTime().ToString("o");seconds=((Get-Date)-$artStart).TotalSeconds;gpu_slot=[bool]$Gpu;appdata=$env:APPDATA} | ConvertTo-Json -Depth 6 | Set-Content "$artLog.job.json"
