...
context=depot/'build/art07/b4/worktree/build/art07/b4/v01/handoff-v01/source/models'
for path in sorted((kit/'models').glob('*.glb'))+[context/'ground.glb',context/'water.glb']:
...
for n in ['kit.json','review.gd','present.gd','surface.gdshader']:
    shutil.copy2(recipe/n,out/n)
...
(out/'project.godot').write_text('config_version=5\n[application]\nconfig/name="ART-07C1 Fen resource review"\nrun/main_scene="res://review.tscn"\n[display]\nwindow/size/viewport_width=1440\nwindow/size/viewport_height=900\nwindow/vsync/vsync_mode=0\n[rendering]\nrenderer/rendering_method="forward_plus"\n')
(out/'review.tscn').write_text('[gd_scene load_steps=2 format=3]\n[ext_resource type="Script" path="res://review.gd" id="1"]\n[node name="C1Review" type="Node3D"]\nscript=ExtResource("1")\n')
(out/'cook.json').write_text(json.dumps({'models':audit,'textures':textures,'unique_texture_mip_bytes':sum(t['rgba8_mip_bytes'] for t in textures.values()),'derivative':'B4 content-hash texture sharing; unchanged geometry buffers and retained ground/water'},indent=2)+'\n')
print('C1_PREPARE_OK',len(audit),len(textures))
