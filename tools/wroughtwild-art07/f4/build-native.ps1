...
$f4Worker=[IO.Path]::GetFullPath((Join-Path $PSScriptRoot '../../..'))
if (-not $artOutput.StartsWith((Join-Path $f4Worker 'build/art07/f4')+[IO.Path]::DirectorySeparatorChar)) { throw 'Output must remain in the F4 build tree.' }
...
project(art07_f4_native LANGUAGES CXX)
...
Write-Output 'F4_NATIVE_BUILD_OK'
