...
if sys.argv[1]=='--motion-only':
    ...
    print('D4_MOTION_ENCODED',dest)
    raise SystemExit(0)

version,review,dest=map(lambda x:Path(x).resolve(),sys.argv[1:])
dest.mkdir(parents=True,exist_ok=False)
names=['blender-overview.png','blender-wood.png','blender-bog_oak.png','blender-resinheart.png','blender-woven_reed.png','blender-corkbark.png']
for name in names:shutil.copy2(version/'blender'/name,dest/name)
for renderer in ['forward_plus','gl_compatibility']:
    for name in ['overview','wood','pine','bog_oak','ash_wood','resinheart','woven_reed','corkbark','shade','unlit']:
...
print('D4_EVIDENCE_OK',len(files))
