"""Install D2 only into a frozen disposable game; refuse normal checkout paths."""
...
snap=a.snapshot.resolve(); assert snap.is_relative_to(root/'build/art07/d2')
target=snap/'game/art07_d2'; assert not target.exists(); target.mkdir()
shutil.copytree(a.models/'assets',target/'assets')
prerequisite=json.loads((snap.parent/'prerequisites.json').read_text())['prerequisite']
shutil.copytree(Path(prerequisite['path'])/'review/game/art07_d1/assets',target/'d1-assets')
for name in ['adapter.gd','checks.gd','gallery.gd']: shutil.copyfile(script/name,target/name)
for name in ['checks','gallery']:
    (target/(name+'.tscn')).write_text('[gd_scene load_steps=2 format=3]\n[ext_resource type="Script" path="res://art07_d2/'+name+'.gd" id="1"]\n[node name="D2" type="Node3D"]\nscript = ExtResource("1")\n')
...
text=text.replace(needle,needle+'\tif String(shape_id) in preload("res://art07_d2/adapter.gd").IDS or String(shape_id) in preload("res://art07_d2/adapter.gd").D1_IDS:\n\t\treturn preload("res://art07_d2/adapter.gd").mesh_for(String(shape_id))\n',1)
...
(snap/'captures/home-reliability').mkdir(parents=True,exist_ok=True)
print('D2_ISOLATED_ADAPTER_INSTALLED',snap)
