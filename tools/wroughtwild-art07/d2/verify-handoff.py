...
print('D2_CANONICAL_HASHES_OK',len(manifest['files']))
if a.output:
    root=Path(__file__).resolve().parents[3]; out=a.output.resolve()
    assert out.is_relative_to(root/'build/art07/d2') and not out.exists()
    out.mkdir(parents=True)
    shutil.copytree(package/'review',out/'review')
    source=out/'source'; source.mkdir()
    shutil.copy2(package/'editable/roof-joinery-spans.blend',source/'roof-joinery-spans.blend')
    shutil.copy2(package/'geometry.json',source/'geometry.json')
    shutil.copytree(package/'review/game/art07_d2/assets',source/'assets')
    ...
    def job(name,args,exe=godot):
        ...
        (jobs/(name+'.json')).write_text(json.dumps({'executable':exe,'arguments':args,'cwd':str(root),'output':str(out/'logs'/name),'gpu':True,'allow_headless_peers':True,'timeout_seconds':600,'appdata':str(out/'isolated-appdata')},indent=2)+'\n')
    base=['--path',str(out/'review/game'),'--audio-driver','Dummy']
    job('import',base+['--headless','--import'])
    job('placement',base+['--headless','res://art07_d2/checks.tscn'])
    job('restart',base+['--headless','res://art07_d2/checks.tscn','--','--d2-restore'])
    for renderer in ['forward_plus','gl_compatibility']:
        job(renderer,base+['--rendering-method',renderer,'--resolution','1440x900','--position','-9999,-9999','res://art07_d2/gallery.tscn','--','--capture'])
    job('blender',['--background','--threads','8','--python-exit-code','1','--python',str(root/'tools/wroughtwild-art07/d2/reopen.py'),'--',str(source),str(out/'blender-reopen')],blender)
    ...
    print('D2_FRESH_VERIFY_COPY_READY',out)
