"""Install D3 only into a frozen disposable game; refuse normal checkout paths."""
...
snap=a.snapshot.resolve(); assert snap.is_relative_to(root/'build/art07/d3')
target=snap/'game/art07_d3'; assert not target.exists(); target.mkdir()
shutil.copytree(a.models/'assets',target/'assets')
prereq=json.loads((snap.parent/'prerequisites.json').read_text())
for k in ['cube','wall_panel','pillar','beam','floor_slab']:
    shutil.copy2(Path(prereq['d1_package'])/'review/game/art07_d1/assets'/f'{k}.glb',target/'assets'/f'{k}.glb')
for name in ['adapter.gd','checks.gd','gallery.gd']: shutil.copyfile(script/name,target/name)
for name in ['checks','gallery']:
    (target/(name+'.tscn')).write_text('[gd_scene load_steps=2 format=3]\n[ext_resource type="Script" path="res://art07_d3/'+name+'.gd" id="1"]\n[node name="D3" type="Node3D"]\nscript = ExtResource("1")\n')
...
text=text.replace(needle,needle+'\tif String(shape_id) in preload("res://art07_d3/adapter.gd").IDS or String(shape_id) in preload("res://art07_d3/adapter.gd").COARSE:\n\t\treturn preload("res://art07_d3/adapter.gd").mesh_for(String(shape_id),String(family))\n',1)
...
print('D3_ISOLATED_ADAPTER_INSTALLED',snap)
