...
print('D3_CANONICAL_HASHES_OK',len(manifest['files']))
if a.output:
    root=Path(__file__).resolve().parents[3]; out=a.output.resolve()
    assert out.is_relative_to(root/'build/art07/d3') and not out.exists()
    out.mkdir(parents=True)
    shutil.copytree(package/'review',out/'review')
    source=out/'source'; source.mkdir()
    shutil.copy2(package/'editable/fine-coverings.blend',source/'fine-coverings.blend')
    shutil.copy2(package/'geometry.json',source/'geometry.json')
    shutil.copytree(package/'review/game/art07_d3/assets',source/'assets')
    ...
    def job(name,args,exe=godot):
        ...
        (jobs/(name+'.json')).write_text(json.dumps({'executable':exe,'arguments':args,'cwd':str(root),'output':str(out/'logs'/name),'gpu':True,'timeout_seconds':600,'appdata':str(out/'isolated-appdata'),'allow_native_dummy_material_diagnostic':name in ['placement','restart']},indent=2)+'\n')
    base=['--path',str(out/'review/game'),'--audio-driver','Dummy']
    job('import',base+['--headless','--import'])
    job('placement',base+['--headless','res://art07_d3/checks.tscn'])
    job('restart',base+['--headless','res://art07_d3/checks.tscn','--','--d3-restore'])
    for renderer in ['forward_plus','gl_compatibility']:
        job(renderer,base+['--rendering-method',renderer,'--resolution','1440x900','--position','-9999,-9999','res://art07_d3/gallery.tscn','--','--capture'])
    job('blender',['--background','--threads','8','--python-exit-code','1','--python',str(root/'tools/wroughtwild-art07/d3/reopen.py'),'--',str(source),str(out/'blender-reopen')],blender)
    ...
    print('D3_FRESH_VERIFY_COPY_READY',out)
