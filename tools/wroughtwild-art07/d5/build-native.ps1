...
$d5Worker=[IO.Path]::GetFullPath((Join-Path $PSScriptRoot '../../..'))
if (-not $artOutput.StartsWith((Join-Path $d5Worker 'build/art07/d5')+[IO.Path]::DirectorySeparatorChar)) { throw 'Output must remain in the D5 build tree.' }
...
project(art07_d5_native LANGUAGES CXX)
...
Write-Output 'D5_NATIVE_BUILD_OK'
