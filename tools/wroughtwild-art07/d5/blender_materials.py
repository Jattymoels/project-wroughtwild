...
def material(c, role, textures):
    ...
    m = bpy.data.materials.new('d5_'+c['id']+'_'+role)
    ...
    if c['id']=='cinderglass' and role=='face':

        p.inputs['Alpha'].default_value=c['opacity']
    ...
    for kind in ['albedo','normal','orm']:
        ...
        tex.image = bpy.data.images.load(str(textures/f'd5_{c["id"]}_{role}_{kind}.png'),check_existing=True)
...
def box(name, size, at, c, mats, grain_axis, collection, uv_offset=(0,0)):
    ...
    if c['id']=='charcoal':
        ...
        # Uneven cut faces keep fuel angular without identical cube silhouettes.

        verts=[(vx*(.79+.19*math.sin(k*4.1+len(name))),vy*(.83+.15*math.cos(k*2.7)),vz*(.88+.1*math.sin(k*1.9))) for k,(vx,vy,vz) in enumerate(verts)]
    ...
    uv=mesh.uv_layers.new(name='D5_Metres')
    ...
    for poly in mesh.polygons:
        ...
        if c['id'] in ['stone','fieldstone','slate','shellstone','rustclay_brick','vitrified_basalt','cinderglass','charcoal']:
    ...
    bevel=obj.modifiers.new('Small physical edge bevel','BEVEL');bevel.width=min(size)*.04;bevel.segments=1
...
def build(textures,out):
    ...
    cfg=json.loads((textures/'textures.json').read_text(encoding='utf-8'))['settings']
    ...
    for i,c in enumerate(cfg['families']):
        ...
        if c['id']=='charcoal':
            ...
            # Inspection of fuel chunks only: no wall-shaped charcoal sample.

            for k in range(9):

                size=(.18+(k%3)*.022,.15+(k%2)*.04,.16+(k%4)*.025)

                o=box('charcoal_fuel_%02d'%k,size,(x+(k%3-1)*.24,-.10+(k//3-1)*.22,.12+(k%2)*.10),c,mats,2,runtime)

                o.rotation_euler[2]=k*.47

        elif c['id']=='cinderglass':

            # Exact current fixed-window envelope and integral frame.

            w=cfg['window']['frame_width_m'];m=cfg['window']['muntin_width_m'];depth=.16

            box('cinderglass_pane',(.85,depth*.24,.85),(x,.3,1),c,[mats[0],mats[0]],2,runtime)

            for side in [-1,1]:

                box('cinderglass_frame_side_'+str(side),(w,depth,1),(x+side*(1-w)/2,.3,1),c,[mats[1],mats[1]],2,runtime)

                box('cinderglass_frame_rail_'+str(side),(.85,depth,w),(x,.3,1+side*(1-w)/2),c,[mats[1],mats[1]],2,runtime)

            box('cinderglass_muntin_v',(m,depth*.8,.85),(x,.3,1),c,[mats[1],mats[1]],2,runtime)

            box('cinderglass_muntin_h',(.85,depth*.8,m),(x,.3,1),c,[mats[1],mats[1]],2,runtime)
            ...
            if c['id']=='slate':
                ...
                for row in range(7):
                    ...
                    box('slate_layer_%02d'%row,(1,.065,.155),(x,.3-row*.007,.58+row*.14),c,mats,2,runtime,uv_offset=(0,row*.14))

            else:

                height=.5 if c['id']=='fieldstone' else 1

                box(c['id']+'_face_coupon',(1,.25,height),(x,.3,.5+height/2),c,mats,2,runtime)

            box(c['id']+'_cut_coupon',(1,.4,.20),(x,-.45,.12),c,mats,2,runtime)

            box(c['id']+'_repeat_coupon',(.5,.10,.5),(x,-.02,1.87),c,mats,2,runtime,uv_offset=(.5,.5))

        text_label(c['name'].upper(),(x,-.84,.025),.10,review)
    ...
    for name,loc,power,size in [('Key',(-3,-5,8),1000,8),('Fill',(6,-2,5),600,7),('Rim',(0,4,6),750,6)]:
    ...
    bpy.ops.export_scene.gltf(filepath=str(out/'d5_proxies.glb'),export_format='GLB',use_selection=True,export_yup=True,export_cameras=False,export_lights=False,export_materials='EXPORT',export_extras=True)

    bpy.ops.file.pack_all()

    render(scene,out/'blender-overview.png',(4,-13,7),(0,0,1.0),13.4)
    ...
    for angle,loc in [('front',(0,-16,2)),('back',(0,16,2)),('side',(14,-9,5)),('top',(0,-.01,19)),('underside',(0,-10,-4))]:

        scene.render.resolution_x=2200;scene.render.resolution_y=950

        render(scene,out/('blender-'+angle+'.png'),loc,(0,0,1),13.4)

    scene.render.resolution_x=2200;scene.render.resolution_y=950

    cam.location=(4,-13,7);aim(cam,(0,0,1));cam.data.ortho_scale=13.4

    bpy.ops.wm.save_as_mainfile(filepath=str(out/'d5_materials.blend'))

    report=audit();report['seconds']=time.perf_counter()-start

    (out/'blender-audit.json').write_text(json.dumps(report,indent=2)+'\n')

    print('D5_BLENDER_OK',report['triangles'],report['seconds'])
...
def reopen(master,out):
    ...
    assert len(result['images'])==48

    assert all(i['packed'] for i in result['images'])

    assert len(result['proxies'])==40

    render(bpy.context.scene,out/'packed-reopen.png',(4,-13,7),(0,0,1),13.4)
    ...
    for angle,loc in [('front',(0,-16,2)),('back',(0,16,2)),('side',(14,-9,5)),('three-quarter',(4,-13,7)),('top',(0,-.01,19)),('underside',(0,-10,-4))]:

        render(bpy.context.scene,out/('clay-'+angle+'.png'),loc,(0,0,1),13.4)

    # Import exported GLB in a separate empty scene and inspect actual export.

    bpy.ops.wm.read_factory_settings(use_empty=True)

    bpy.ops.import_scene.gltf(filepath=str(master.parent/'d5_proxies.glb'))
    ...
    native={s['id']:s for s in json.loads(native_path.read_text(encoding='utf-8'))['shapes']}
    ...
    glass=[o for o in bpy.data.objects if o.type=='MESH' and o.name.startswith('cinderglass_')]
    corners=[o.matrix_world @ Vector(corner) for o in glass for corner in o.bound_box]
    bounds=[max(v[k] for v in corners)-min(v[k] for v in corners) for k in range(3)]
    assert all(abs(a-b)<1e-5 for a,b in zip(bounds,[1,.16,1])),bounds
    result['fixed_window_assembly_bounds_blender_m']=bounds
    assert len(export)==40,len(export)

    result['fresh_glb_import']=export

    (out/'reopen-audit.json').write_text(json.dumps(result,indent=2)+'\n')

    print('D5_REOPEN_OK',len(export))
