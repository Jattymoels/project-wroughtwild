...
assert out.is_relative_to(root/'build/art07/f3');out.mkdir(parents=True,exist_ok=False)
...
for name,sources in [('world',['tests/sim/test_world_intensive.cpp','sim/src/worldgen.cpp','sim/src/worldgen_profiles.cpp','sim/src/tuning.cpp','sim/src/json.cpp','sim/src/lattice.cpp']),('core',['tests/sim/test_main.cpp']+[p.relative_to(root).as_posix() for p in sorted((root/'sim/src').glob('*.cpp'))])]:
