...
root=Path(__file__).resolve().parents[3]/'build/art07/f3'
assert source.is_relative_to(root) and dest.is_relative_to(root)
data=json.loads((source/'manifest.json').read_text());assert not dest.exists();dest.mkdir(parents=True)
...
print('F3_FRESH_COPY_VERIFIED',len(data['files']),dest)
