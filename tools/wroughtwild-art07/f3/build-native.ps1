...
$f3Worker=[IO.Path]::GetFullPath((Join-Path $PSScriptRoot '../../..'))
if (-not $artOutput.StartsWith((Join-Path $f3Worker 'build/art07/f3')+[IO.Path]::DirectorySeparatorChar)) { throw 'Output must remain in the F3 build tree.' }
...
project(art07_f3_native LANGUAGES CXX)
...
Write-Output 'F3_NATIVE_BUILD_OK'
