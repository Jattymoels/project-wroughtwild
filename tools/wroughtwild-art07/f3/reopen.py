...
bpy.ops.wm.open_mainfile(filepath=str(source/'f3_master.blend'))
...
for p in sorted(source.glob('*.glb')):
 ...
 for o in objects:
  ...
  if p.stem in [k+'_'+level for k in ['pullstone','ventlung'] for level in ['near','middle','far']]:assert boundary==0 and nonmanifold==0,(p,topology)
...
print('F3_PACKED_REOPEN_OK',len(images),len(report['exports']))
