param([string]$Revision='56ce6bbe343012205690cf669491372958b80662', [Parameter(Mandatory)][string]$Output, [Parameter(Mandatory)][string]$Depot)
...
if (-not $artOutput.StartsWith((Join-Path $d1Worker 'build/art07/d1')+[IO.Path]::DirectorySeparatorChar)) { throw 'Output must remain in the D1 build tree.' }
...
project(art07_d1_native LANGUAGES CXX)
...
Write-Output 'D1_NATIVE_BUILD_OK'
