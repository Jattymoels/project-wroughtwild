...
def job(name,args,gpu=False,executable=godot,timeout=600):
    data={'executable':executable,'arguments':args,'cwd':str(root),'output':str(build/'logs'/name),'gpu':gpu,'timeout_seconds':timeout,'appdata':str(build/'isolated-appdata')}
    (jobs/(name+'.json')).write_text(json.dumps(data,indent=2)+'\n')
base=['--path',str(game),'--audio-driver','Dummy']
job('import',base+['--headless','--import'])
job('d1-check',base+['--headless','res://art07_d1/checks.tscn'])
job('d1-restart',base+['--headless','res://art07_d1/checks.tscn','--','--d1-restore'])
for renderer in ['forward_plus','gl_compatibility']:
    for mode in ['capture','benchmark']:
        job(renderer+'-'+mode,base+['--rendering-method',renderer,'--resolution','1440x900','--position','-9999,-9999','res://art07_d1/gallery.tscn','--','--'+mode],True)
job('reopen',['--background','--threads','8','--python-exit-code','1','--python',str(root/'tools/wroughtwild-art07/d1/reopen.py'),'--',str(models),str(build/'blender-reopen')],False,blender)
...
print('D1_JOB_SPECS_WRITTEN',jobs)
