param([string]$Revision='4b5d89b376765fbf4d46049aa099e0bb154a82da', [Parameter(Mandatory)][string]$Output, [Parameter(Mandatory)][string]$Depot)
$ErrorActionPreference='Stop'
$artRepo=[IO.Path]::GetFullPath($Depot)
$artOutput=[IO.Path]::GetFullPath($Output)
$f2Worker=[IO.Path]::GetFullPath((Join-Path $PSScriptRoot '../../..'))
if (-not $artOutput.StartsWith((Join-Path $f2Worker 'build/art07/f2')+[IO.Path]::DirectorySeparatorChar)) { throw 'Output must remain in the F2 build tree.' }
...
project(art07_f2_native LANGUAGES CXX)
...
Write-Output 'F2_NATIVE_BUILD_OK'
