...
$e3Worker=[IO.Path]::GetFullPath((Join-Path $PSScriptRoot '../../..'))
if (-not $artOutput.StartsWith((Join-Path $e3Worker 'build/art07/e3')+[IO.Path]::DirectorySeparatorChar)) { throw 'Output must remain in the E3 build tree.' }
...
project(art07_e3_native LANGUAGES CXX)
...
Write-Output 'E3_NATIVE_BUILD_OK'
