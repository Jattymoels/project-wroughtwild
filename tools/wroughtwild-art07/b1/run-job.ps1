...
if(-not $artLog.StartsWith($artRoot+[IO.Path]::DirectorySeparatorChar)){throw 'Log must be in B1 output.'}
...
try {
 ...
 Write-Output "B1 own PID $($artProc.Id)"
 while(-not $artProc.WaitForExit(1000)){
  if(Select-String -Path $artLog,"$artLog.stderr" -Pattern 'SCRIPT ERROR|SHADER ERROR|Parse Error|Traceback' -Quiet){Stop-Process -Id $artProc.Id; $artProc.WaitForExit();throw "Stopped this B1 PID after script/shader failure: $artLog"}
 }
 $artProc.Refresh()
