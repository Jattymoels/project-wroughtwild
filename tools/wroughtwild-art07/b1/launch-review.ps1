param([Parameter(Mandatory)][string]$Package,[Parameter(Mandatory)][string]$CopyTo,[ValidateSet('forward_plus','gl_compatibility')][string]$Renderer='forward_plus',[ValidateSet('check','capture','motion','review')][string]$Mode='check',[switch]$Visible)
...
try {
 ...
 foreach($phase in @('import','review')){
  ...
  Write-Output "B1 copy own PID $($artProc.Id)"
...
Write-Output "B1_FRESH_HANDOFF_OK $artCopy"
