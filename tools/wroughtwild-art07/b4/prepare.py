...
for n in ['scene.json','review.gd','surface.gdshader']:shutil.copy2(recipe/n,out/n)
for n in ['layout.json','models.json']:shutil.copy2(kit/n,out/n)
...
(out/'art').mkdir()
for n in ['environment_sound.gd','environment_sound_look.gd','environment_sound_look.tres']:shutil.copy2(depot/'game/art'/n,out/'art'/n)
(out/'project.godot').write_text('''config_version=5
[application]
config/name="ART-07B4 Composed walk"
...
print('B4_COOK_OK',len(audit),len(textures))
