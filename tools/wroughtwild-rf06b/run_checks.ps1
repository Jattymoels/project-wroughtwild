param(
 [ValidateSet('import','placement','walk','continue','look')][string]$Job='placement',
...
$rfOutput="$rfRoot/build/rf06b"
foreach($dir in @($rfOutput,"$rfOutput/user","$rfOutput/local","$rfOutput/temp","$rfOutput/logs","$rfOutput/media")) {
 New-Item -ItemType Directory -Path $dir -Force | Out-Null
}
$rfPrior=@{};foreach($key in @('APPDATA','LOCALAPPDATA','TEMP','TMP','WROUGHTWILD_RF06B_OUTPUT')){$rfPrior[$key]=[Environment]::GetEnvironmentVariable($key,'Process')}
# Focused RF06B phases; the retained V7 save is a private read-only source copy.
$env:APPDATA="$rfOutput/user"
$env:LOCALAPPDATA="$rfOutput/local"
$env:TEMP="$rfOutput/temp"
$env:TMP="$rfOutput/temp"
$env:WROUGHTWILD_RF06B_OUTPUT=$rfOutput
$mutex=$null
$held=$false
$madeOverride=$false
$process=$null
...
try {
 ...
 if($Job -eq 'import') {
  ...
  $arguments+="res://tests/rf06b/$Job.tscn"
  if($Job -in @('walk','look')) {
