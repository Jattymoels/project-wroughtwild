"""Select actual engine captures; GIF includes a disclosed cut from fen to lake."""
from pathlib import Path
import json, shutil
from PIL import Image
root=Path(__file__).resolve().parents[2]
out=root/'build/rf06b'
report=json.loads((out/'walk-checks.json').read_text())
assert report['failures']==0
media=out/'media'
evidence=root/'docs/prototype/rf06b-evidence-2026-09-16'
evidence.mkdir(parents=True,exist_ok=True)
for name in ['01-fen-ground.png','02-fen-opening.png','05-planted-bank.png']:
...
if frames:
    ...
    frames[0].save(evidence/'fen-shore-walk.gif',save_all=True,append_images=frames[1:],duration=([60,70,70]*40)[:len(frames)],loop=0,optimize=False)
