param(
 ...
 [string]$Output='D:/Wroughtwild/work/mob04-beetle/build/mob04/integration',
 [string]$Godot='C:/Users/Matty/Godot/Godot_v4.5-stable_win64_console.exe',
 [switch]$Capture
)
$ErrorActionPreference='Stop'
$beetleRoot=(Resolve-Path (Join-Path $PSScriptRoot '../..')).Path.Replace('\','/')
...
$env:WROUGHTWILD_BEETLE_OUTPUT=$Output
$mutex=$null
$held=$false
$madeOverride=$false
$process=$null
$resultCode=1
$override="$beetleRoot/game/override.cfg"
$overrideText="[display]`nwindow/size/no_focus=true`nwindow/size/viewport_width=1024`nwindow/size/viewport_height=768`n"
$started=Get-Date
try {
 $arguments=@('--path',"$beetleRoot/game",'--audio-driver','Dummy')
 if($Job -eq 'import') {
  $arguments+=@('--headless','--editor','--import')
 } else {
  $arguments+="res://tests/mob04/$Job.tscn"
  if($Capture) {
   ...
   $madeOverride=$true
   $arguments+=@('--rendering-method','forward_plus','--fixed-fps','60','--','--r8-no-mouse-capture','--beetle-capture')
