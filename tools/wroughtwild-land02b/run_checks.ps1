param(
 [ValidateSet('import','use','continue','early','second','generation','contact')][string]$Job='continue',
...
$rfOutput="$rfRoot/build/land02b"
foreach($dir in @($rfOutput,"$rfOutput/user","$rfOutput/local","$rfOutput/temp","$rfOutput/logs","$rfOutput/media")) {
 New-Item -ItemType Directory -Path $dir -Force | Out-Null
}
$rfPrior=@{};foreach($key in @('APPDATA','LOCALAPPDATA','TEMP','TMP','WROUGHTWILD_LAND02B_OUTPUT')){$rfPrior[$key]=[Environment]::GetEnvironmentVariable($key,'Process')}
# Focused LAND02B phases use only private LAND02B fixtures and output directories.
$env:APPDATA="$rfOutput/user"
$env:LOCALAPPDATA="$rfOutput/local"
$env:TEMP="$rfOutput/temp"
$env:TMP="$rfOutput/temp"
$env:WROUGHTWILD_LAND02B_OUTPUT=$rfOutput
$mutex=$null
$held=$false
$madeOverride=$false
$process=$null
...
try {
 ...
 if($Job -eq 'import') {
  ...
  $arguments+="res://tests/land02b/$Job.tscn"
  if($Job -in @('use','early','second','contact')) {
