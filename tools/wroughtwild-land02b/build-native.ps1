...
if($LASTEXITCODE -ne 0){throw 'LAND02B native configure failed'}
& $rfCmake --build "$rfOut/native" -j 6 *> "$rfOut/native/build.log"
if($LASTEXITCODE -ne 0){Get-Content "$rfOut/native/build.log" -Tail 35;throw 'LAND02B native build failed'}
...
$rfFiles+=@('sim/src/worldgen_frontier_v7.inc','tools/wroughtwild-land02b/CMakeLists.txt','tools/wroughtwild-land02b/build-native.ps1')
...
Write-Output "LAND02B_BUILT $rfDll"
