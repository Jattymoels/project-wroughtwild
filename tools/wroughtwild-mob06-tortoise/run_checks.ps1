param(
 ...
 [string]$Output='D:/Wroughtwild/work/mob06-tortoise/build/mob06/integration',
 [string]$Godot='C:/Users/Matty/Godot/Godot_v4.5-stable_win64_console.exe',
 [switch]$Capture,
 [switch]$CaptureOnly
)
$ErrorActionPreference='Stop'
if($CaptureOnly -and (-not $Capture -or $Job -ne 'native')) {throw 'CaptureOnly requires native -Capture.'}
$tortoiseRoot=(Resolve-Path (Join-Path $PSScriptRoot '../..')).Path.Replace('\','/')
...
$env:WROUGHTWILD_TORTOISE_OUTPUT=$Output
$mutex=$null
$held=$false
$madeOverride=$false
$process=$null
$resultCode=1
$override="$tortoiseRoot/game/override.cfg"
$overrideText="[display]`nwindow/size/no_focus=true`nwindow/size/viewport_width=1024`nwindow/size/viewport_height=768`n"
$started=Get-Date
try {
 $arguments=@('--path',"$tortoiseRoot/game",'--audio-driver','Dummy')
 if($Job -eq 'import') {
  $arguments+=@('--headless','--editor','--import')
 } else {
  $arguments+="res://tests/mob06/$Job.tscn"
  if($Capture) {
   ...
   $madeOverride=$true
   $arguments+=@('--rendering-method','forward_plus','--fixed-fps','60','--','--r8-no-mouse-capture','--tortoise-capture')
   if($CaptureOnly) {$arguments+='--tortoise-capture-only'}
 ...
 # Cache the native process handle before WaitForExit (Windows PowerShell).
 ...
 $process.WaitForExit()
 if($null -eq $process.ExitCode) {throw "Owned $Job exit code was unavailable; do not mark it passed."}
