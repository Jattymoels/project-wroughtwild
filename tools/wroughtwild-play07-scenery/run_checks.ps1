param(
 [ValidateSet('after','lifecycle','retire','device')][string]$Job='after',
...
$rfOutput="$rfRoot/build/play07-scenery"
...
try {
 ...
 if($Job -eq 'import') {
  ...
  $arguments+= $(if($Job -eq 'retire'){'res://tests/play07_scenery_retire.tscn'}elseif($Job -in @('lifecycle','device')){'res://tests/play07_scenery_lifecycle.tscn'}else{'res://tests/play07_scenery.tscn'})
  if($Job -in @('after','retire')) {
  ...
  } else {$arguments+=@('--headless','--','--r8-no-mouse-capture','--arrival-lifecycle');if($Job -eq 'device'){$arguments+='--scenery-device-only'}}
 }
