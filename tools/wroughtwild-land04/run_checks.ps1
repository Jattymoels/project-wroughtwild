param(
 [ValidateSet('import','use','continue','early','generation','corrected_route')][string]$Job='continue',
...
$rfOutput="$rfRoot/build/land04"
foreach($dir in @($rfOutput,"$rfOutput/user","$rfOutput/local","$rfOutput/temp","$rfOutput/logs","$rfOutput/media")) {
 New-Item -ItemType Directory -Path $dir -Force | Out-Null
}
$rfPrior=@{};foreach($key in @('APPDATA','LOCALAPPDATA','TEMP','TMP','WROUGHTWILD_LAND04_OUTPUT')){$rfPrior[$key]=[Environment]::GetEnvironmentVariable($key,'Process')}
# Focused LAND04 phases use only private LAND04 fixtures and output directories.
$env:APPDATA="$rfOutput/user"
$env:LOCALAPPDATA="$rfOutput/local"
$env:TEMP="$rfOutput/temp"
$env:TMP="$rfOutput/temp"
$env:WROUGHTWILD_LAND04_OUTPUT=$rfOutput
$mutex=$null
$held=$false
$madeOverride=$false
$process=$null
...
$rfLabel=if($RoutesOnly){'routes'}else{$Job}
...
try {
 ...
 if($Job -eq 'import') {
  ...
  $arguments+="res://tests/land04/$Job.tscn"
  if($Job -in @('use','early','corrected_route')) {
   ...
   if($RoutesOnly){$arguments+='--land04-routes-only'}
