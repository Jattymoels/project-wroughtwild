"""Original LAND-04 growth and close bank kit, Blender 4.5 background recipe.
...
OUT=ROOT/'game/land04/assets'
SOURCE=ROOT/'build/land04/source-art'
OUT.mkdir(parents=True,exist_ok=True); SOURCE.mkdir(parents=True,exist_ok=True)
bpy.ops.object.select_all(action='SELECT'); bpy.ops.object.delete(use_global=False)
mat=bpy.data.materials.new('LAND04 original host vertex materials');mat.use_nodes=True
...
def sprig(m,root,angle,height,rng,dry=False):
    ...
    for j in range(2,7):
        ...
        for s in [-1,1]:
            ...
            leaf(m,at,direction,.22 if dry else .40,.065 if dry else .10,(.17,.205,.093) if dry else (.105,.18,.06),detail=0)
...
def root_graph(m,rng,green=True,low=False):
    ...
    bark=(.135,.097,.054) if not low else (.18,.128,.072)
    glow=(.035,.49,.16) if green else (.64,.69,.59)
    width=2.4 if not low else 2.1;length=3.7 if not low else 3.2
    trunk=[Vector((.18*math.sin(t*3),-length*.5+length*t,.085+.06*math.sin(t*math.pi))) for t in [0,.2,.4,.6,.8,1]]
    tube(m,trunk,[.13,.14,.12,.105,.075,.024] if not low else [.075,.09,.08,.06,.04,.016],bark,sides=10)
    ...
    for j in range(1,5):
        ...
        for sign in [-1,1]:
            ...
            tube(m,path,[.09,.075,.047,.009] if not low else [.055,.045,.025,.005],bark,sides=9)
            ...
            tube(m,[fork,fork.lerp(tip,.55)+Vector((0,0,.04)),tip],[.035,.023,.004],bark,sides=7)
            ribbon(m,[fork+Vector((0,0,.034)),tip+Vector((0,0,.005))],.006,glow,phase=j/5+.7)
            for k in range(4 if low else 3):
                at=end+Vector((rng.uniform(-.21,.21),rng.uniform(-.21,.21),-.04))
                # Woodland fans carry upright fronds above the connected roots;
                # dry Steppe colonies remain distinctly low and small-leaved.
                sprig(m,at,rng.random()*math.tau,rng.uniform(.18,.48) if low else rng.uniform(1.05,1.95) if green else rng.uniform(.42,.90),rng,dry=low)
...
for slot,(role,purpose) in enumerate(roles.items()):
    ...
    if role in ['blue-sheaths','blue-bracts-dry']:
        ...
        for plant in range(4 if not dry else 3):
            ...
            for tier in range(3):
                for k in range(7-tier):
                    ...
                    sheath(m,root+Vector((0,0,tier*.12)),a,1.1*scale,.22*scale,(1.22+tier*.17)*scale,c,k+tier,rng)
        ...
        for bank in range(4):
            for row in range(3):
                ...
                clast(m,at,.70-rng.random()*.10,.45,.10+row*.025,rng,(.23,.265,.24))
                if bank%3!=2:
        ...
        for k in range(7):
            ...
            at=(-1.9+k*.58,math.sin(k)*.48,.025)
            clast(m,at,.43,.39,.16,rng,(.25,.25,.20))
    else:root_graph(m,rng,green=True,low=role=='green-steppe-colony')
    ob=m.obj('LAND04 '+role,mat);ob['purpose']=purpose;ob['support']='Native actual terrain triangles; no stock or terrain owner'
...
document={'method':'Original directly authored procedural Blender geometry, not image-to-3D or concept pixels.','recipe':'tools/wroughtwild-land04/build_hosts.py','seed':170904,'source':str(SOURCE/'force-hosts.blend'),'coordinates':'Metres; standard glTF Y up.','forms':report}
(OUT.parent/'source.json').write_text(json.dumps(document,indent=2)+'\n',encoding='utf8')
(SOURCE/'source.json').write_text(json.dumps(document,indent=2)+'\n',encoding='utf8')
print('LAND04_ART '+json.dumps(document))
