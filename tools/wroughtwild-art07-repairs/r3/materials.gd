...
static func enabled() -> bool:
	return not "--r3-baseline" in OS.get_cmdline_user_args()
...
static func _load_families() -> void:
	...
	var construction: Dictionary = JSON.parse_string(FileAccess.get_file_as_string("res://../data/tuning/construction.json"))
...
static func material_for(family: String, role: String, shape_id: String = "trim", surface: int = 0) -> Material:
	...
	for map in ["albedo", "normal", "orm"]:
		mat.set_shader_parameter("face_"+map, load(stem+("" if slice=="d6" else "_face")+"_"+map+".png"))
		mat.set_shader_parameter("edge_"+map, load(stem+("" if slice=="d6" else "_edge")+"_"+map+".png"))
...
static func binding_inventory() -> Dictionary:
	...
	var active: Dictionary = cache if enabled() else G1Materials.cache
