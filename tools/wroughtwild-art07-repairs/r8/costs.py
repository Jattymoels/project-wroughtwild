"""Validate repeated R8 observations and write an encoding-safe cost derivative."""
import argparse
import statistics
from pathlib import Path
from inspect_inputs import ROOT,read
from compose import write
BUILD=ROOT/"build/art07-repairs/r8"
...
def reports(version,mode,renderer):
    found=[]
    for path in sorted((BUILD/version/'runtime/evidence').glob('benchmark-'+mode+'-'+renderer+'-*/report.json')):
        result=read(path)
...
def main(before,after,out):
    ...
    lines=['# ART-07R8 measured costs','',result['conditions'],'']
    for renderer in ['forward_plus','gl_compatibility']:
        groups={'art-off':reports(before,'baseline',renderer),'G1':reports(before,'art',renderer),'R8':reports(after,'art',renderer),'R8 art-off':reports(after,'baseline',renderer)}
        ...
        old=data['modes']['G1']['setup_seconds'];new=data['modes']['R8']['setup_seconds']
        data['median_setup_reduction_percent']=100*(1-new['median']/old['median'])
        data['setup_observed_ranges_do_not_overlap']=new['max']<old['min']
        lines += ['',f"Observed median setup reduction: {data['median_setup_reduction_percent']:.2f}%. Every R8 run below every G1 run: {data['setup_observed_ranges_do_not_overlap']}.",'','| Loaded cost | Art-off range | G1 range | R8 range | R8 art-off range |','| --- | --- | --- | --- | --- |']
    ...
    print('R8_COST_COMPARISON_VALID',str(out))
