"""Prepare Green review using the recorded frozen ART-04 native binary.
...
checkpoint=repo/'build/lf2/published-b/lf2-green-pending.json'
shutil.copy2(checkpoint,review/'paid-checkpoint.json')
for p in recipe.iterdir():
    if p.suffix in ['.gd','.gdshader','.godot','.tscn'] or p.name=='green.json':shutil.copy2(p,review/p.name)
if assets.is_dir():
    for p in assets.iterdir():
        if p.suffix in ['.png','.glb'] or p.name=='asset-report.json':shutil.copy2(p,review/p.name)
(review/'context').mkdir(exist_ok=True)
for name in ['strange_winch','strange_basket','strange_drum','strange_lever','strange_ventlung','forge_basic']:
    path=repo/'game/assets/authored'/f'{name}.glb';assert path.is_file(),path
    shutil.copy2(path,review/'context'/path.name)
shutil.copy2(repo/'build/workshop-white/assets-v06/white-post-mid.glb',review/'context/white-post.glb')
for role in ['base','orm','normal','scar']:
    shutil.copy2(repo/'build/workshop-white/assets-v06'/('white-'+role+'.png'),review/'context'/('white-'+role+'.png'))
shutil.copy2(repo/'build/workshop-blue/assets-v03/blue-post-mid.glb',review/'context/blue-post.glb')
for role in ['base','orm','normal','scar']:
    shutil.copy2(repo/'build/workshop-blue/assets-v03'/('blue-'+role+'.png'),review/'context'/('blue-'+role+'.png'))
...
print('GREEN_REVIEW_PREPARED',review)
