...
for kind,limit in [('source',np.array([1.5,1.5,1.1])),('post',np.array([.65,.55,1.18]))]:
    for lod in ['near','mid','far']:
        path=assets/f'green-{kind}-{lod}.glb'
...
for i in range(1,4):
    path=assets/f'green-fragment-{i}.glb'
    bpy.ops.wm.read_factory_settings(use_empty=True);bpy.ops.import_scene.gltf(filepath=str(path))
    ...
    bm.free()
bpy.ops.wm.open_mainfile(filepath=str(assets/'green-workshop.blend'))
check('SOURCE - untouched normalized Green inclusion' in bpy.data.objects,'dense untouched source retained')
check('SOURCE - editable procedural housing' in bpy.data.objects,'editable material authoring retained')
cores=[bpy.data.objects['GREEN_CORE - '+name] for name in ['stem','first','second']]
core_for_fit=cores[0]
bpy.context.view_layer.update()
core_low=min((core_for_fit.matrix_world@v.co).z for v in core_for_fit.data.vertices)
check(-.004<=core_low-(.655+.055/2)<=.002,'resin stem seats on its physical cradle without floating')
def tree(obj):
    obj.data.calc_loop_triangles()
    return BVHTree.FromPolygons([obj.matrix_world@v.co for v in obj.data.vertices],[tuple(t.vertices) for t in obj.data.loop_triangles])
for branch in cores[1:]:check(bool(tree(cores[0]).overlap(tree(branch))),'resin branch physically joins the stem: '+branch.name)
textures=[(name,2048) for name in ['green-base.png','green-orm.png','green-normal.png','green-scar.png','post-base.png','post-orm.png','post-normal.png']]
...
host=bpy.data.objects['Green inclusion - ready worked or spent host'];host.location.x=-1.25
...
for core in cores:core['export_location']=list(core.location);core.location.x+=1.1
shown=[host,housing]+cores
for i in range(3):
    o=bpy.data.objects['Green Mineral - cut fragment '+str(i+1)];o.location=(i*.35-.35,-.8,0);shown.append(o)
...
bpy.ops.wm.save_as_mainfile(filepath=str(out/'green-workshop.blend'),compress=True)
scene.render.filepath=str(out/'editable-preview.png');bpy.ops.render.render(write_still=True)
bpy.ops.wm.open_mainfile(filepath=str(out/'green-workshop.blend'))
for im in bpy.data.images:
    if im.has_data:check(im.packed_file is not None,'packed image '+im.name)
check(len([o for o in bpy.context.scene.objects if o.type=='MESH' and not o.hide_render])==8,'packed source reopens with host, housing, three joined resin cores and three recovered pieces')
report={'checks':checks,'failures':[],'runtime_assets':records,'blender':bpy.app.version_string,'packed_blend_sha256':hashlib.sha256((out/'green-workshop.blend').read_bytes()).hexdigest()}
