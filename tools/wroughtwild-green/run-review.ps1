...
if ($Mode -eq 'open') { Write-Output "GREEN_REVIEW_STARTED $($artProcess.Id)"; return }
...
# Offline art review makes no TLS calls. Windows sandbox certificate-store
# access can emit this startup warning; preserve it in the log while continuing
# to fail every engine/script/shader/assertion error relevant to this review.
...
Write-Output "GREEN_REVIEW_OK $Mode"
