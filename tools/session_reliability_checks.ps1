param(
    ...
    [Alias('Scene')][string[]]$Scenes = @('session_soak'),
    [ValidateSet('frontier_v5','frontier_v6')][string]$Profile = 'frontier_v6',
    [int]$Seed = 1,
    [string]$ReviewName = 'session',
    [switch]$Prepare,
    [switch]$Import,
    [switch]$Rendered,
...
# INT-07C repeated-session and recovery checks, in separate cold
...
$worldRoot = Join-Path $worldRepo 'build/session-reliability'
...
if ($Prepare) {
    if ($Phase -eq 'baseline') { throw 'Baseline must be preserved from 7cfd781 separately; this runner never prepares it from current source.' }
...
foreach ($worldFixture in @('save_recovery','session_soak')) {
...
try {
    ...
    function Invoke-WorldReview([string]$Name, [string]$Arguments) {
        ...
        $worldText -split "`n" | Select-String -Pattern 'checks|failures|FAIL|SCRIPT ERROR|ERROR:|SESSION_|SAVE_RECOVERY|Godot Engine' | ForEach-Object { $_.Line }
