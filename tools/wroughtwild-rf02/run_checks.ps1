param(
 [ValidateSet('import','assets','walk','continue')][string]$Job='assets',
...
$env:WROUGHTWILD_RF02_OUTPUT=$rfOutput
$mutex=$null
$held=$false
$madeOverride=$false
$process=$null
...
try {
 ...
 if($Job -eq 'import') {
  ...
  $arguments+="res://rf02/checks/$Job.tscn"
  if($Job -in @('walk','continue')) {
