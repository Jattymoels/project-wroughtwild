param(
 [ValidateSet('import','use','continue','early','second','generation','diagnostic')][string]$Job='continue',
...
$rfOutput="$rfRoot/build/land02"
foreach($dir in @($rfOutput,"$rfOutput/user","$rfOutput/local","$rfOutput/temp","$rfOutput/logs","$rfOutput/media")) {
 New-Item -ItemType Directory -Path $dir -Force | Out-Null
}
$rfPrior=@{};foreach($key in @('APPDATA','LOCALAPPDATA','TEMP','TMP','WROUGHTWILD_LAND02_OUTPUT')){$rfPrior[$key]=[Environment]::GetEnvironmentVariable($key,'Process')}
# Focused LAND02 phases use only private LAND02 fixtures and output directories.
$env:APPDATA="$rfOutput/user"
$env:LOCALAPPDATA="$rfOutput/local"
$env:TEMP="$rfOutput/temp"
$env:TMP="$rfOutput/temp"
$env:WROUGHTWILD_LAND02_OUTPUT=$rfOutput
$mutex=$null
$held=$false
$madeOverride=$false
$process=$null
...
try {
 ...
 if($Job -eq 'import') {
  ...
  $arguments+="res://tests/land02/$Job.tscn"
  if($Job -in @('use','early','second')) {
