"""Blender: reopen the handoff and check actual deformed near-mesh hoof soles.
...
mesh=bpy.data.objects['Kilnback - near candidate']
mesh.hide_set(False);arm.hide_set(False)
lods={name:bpy.data.objects[object_name] for name,object_name in [('mid','Kilnback - mid candidate'),('far','Kilnback - far rebaked candidate')]}
...
for clip,duration in rig['clips_seconds'].items():
    ...
    for sample in range(samples):
        ...
        assert coordinates[:,2].min()>=-.001,(clip,sample,'body/tusk below floor',float(coordinates[:,2].min()))
        ...
        for name,value in floors.items():
            ...
            assert value<=(.068 if clip=='walk' else .003),(clip,sample,name,'hovering',value)
        lod_minima={}
        ...
        records.append({'clip':clip,'seconds':frame/100,'lowest_surface_metres':float(coordinates[:,2].min()),'lod_lowest_surface_metres':lod_minima,'hoof_sole_y_metres':floors})
...
print('BOAR_SOURCE_AUDIT_OK',flush=True)
