"""Blender: author and bake one boar's scars without changing its source file.

--python build_surface.py -- CONFIG.json NEW_OUTPUT
"""
import hashlib
import json
import math
...
config_path, output = [Path(p).resolve() for p in sys.argv[sys.argv.index('--') + 1:]]
...
boar = next(o for o in bpy.context.scene.objects if o.type == 'MESH')
boar.name = 'Kilnback - authored source'
mesh = boar.data
points = np.array([boar.matrix_world @ v.co for v in mesh.vertices])
rotation = Matrix.Rotation(math.radians(cfg['yaw_degrees']), 4, 'Z')
rotated = np.array([rotation @ Vector(p) for p in points])
lo, hi = rotated.min(0), rotated.max(0)
factor = cfg['height_metres'] / (hi[2] - lo[2])
origin = Vector(((lo[0]+hi[0])/2, (lo[1]+hi[1])/2, lo[2]))
boar.matrix_world = Matrix.Scale(factor,4) @ Matrix.Translation(-origin) @ rotation @ boar.matrix_world
bpy.context.view_layer.objects.active = boar
boar.select_set(True)
bpy.ops.object.transform_apply(location=True, rotation=True, scale=True)
points = np.array([v.co for v in mesh.vertices])
reference = boar.copy(); reference.data = mesh.copy()
reference.name = 'SOURCE - normalized intact boar'
bpy.context.collection.objects.link(reference)
reference.hide_render = True; reference.hide_set(True)
reference.select_set(False)

# Rays follow the actual outer shield/hide. No floating ribbon or tube is added.
...
# Preserve embedded bytes with their actual MIME type, then transcode to PNG.
# This source uses WebP; a .png extension on those bytes fails Godot's importer.
data=raw.read_bytes(); length=struct.unpack_from('<I',data,12)[0]
doc=json.loads(data[20:20+length]); binary=data[28+length:]
for image_index,(im,name) in enumerate(zip(doc['images'],['base.png','orm.png'])):
    view=doc['bufferViews'][im['bufferView']]; offset=view.get('byteOffset',0)
    extension={'image/webp':'.webp','image/png':'.png','image/jpeg':'.jpg'}[im['mimeType']]
    embedded=output/('embedded-'+Path(name).stem+extension)
    embedded.write_bytes(binary[offset:offset+view['byteLength']])
    # glTF's importer already decoded and packed the image. Force its pixels
    # before changing the output path (Blender otherwise loads lazily).
    converted=bpy.data.images['Image_'+str(image_index)].copy()
    assert len(converted.pixels)>0
    converted.filepath_raw=str(output/name);converted.file_format='PNG';converted.save()

material=mesh.materials[0].copy();mesh.materials[0]=material
material.name='Kilnback - buried Red scars'
...
bpy.ops.object.select_all(action='DESELECT');boar.select_set(True);bpy.context.view_layer.objects.active=boar
...
scene['study']='ART-01 isolated boar - not adopted game art'
bpy.ops.wm.save_as_mainfile(filepath=str(output/'boar-surface.blend'),compress=True)
assert hashlib.sha256(raw.read_bytes()).hexdigest()==cfg['source_sha256']
print('BOAR_SURFACE_OK '+str(output),flush=True)
