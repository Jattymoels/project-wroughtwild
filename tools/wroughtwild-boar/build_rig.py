"""Blender: boar deformation/LOD candidate from the reviewed scar surface.
...
cfg=json.loads((repo/'tools/wroughtwild-boar/boar-study.json').read_text())
settings=json.loads((repo/'tools/wroughtwild-boar/rig-study.json').read_text())
bpy.ops.wm.open_mainfile(filepath=str(surface/'boar-surface.blend'))
scene=bpy.context.scene
source=bpy.data.objects['Kilnback - authored source']
source.hide_set(False)
bpy.ops.object.select_all(action='DESELECT')
near=source.copy();near.data=source.data.copy();near.name='Kilnback - near candidate'
...
# Bake the detailed shield/bristles and the recessed mask's normal response.
mat=near.data.materials[0].copy();near.data.materials[0]=mat
normal=bpy.data.images.new('Kilnback baked normals',width=settings['normal_pixels'],height=settings['normal_pixels'],alpha=False)
...
# Articulation fitted to the actual normalized boar, with rigid hooves and head.
p=np.array([v.co for v in near.data.vertices])
bones=[{'name':'body','parent':None,'head':[0,-.12,.66],'tail':[0,.18,.68]},
       {'name':'chest','parent':'body','head':[0,.12,.73],'tail':[0,.34,.75]},
       {'name':'head','parent':'chest','head':[0,.36,.68],'tail':[0,.71,.41]},
       {'name':'tail','parent':'body','head':[0,-.67,.69],'tail':[0,-.78,.59]}]
feet=[]
for front,yhip,zhip in [(True,.20,.66),(False,-.47,.64)]:
    for side in [-1,1]:
        name=('front' if front else 'rear')+('_L' if side<0 else '_R')
        q=p[(p[:,0]*side>.1)&(p[:,1]*(1 if front else -1)>0)&(p[:,1]<.50)&(p[:,2]<.15)]
        assert len(q)>10
        sole=float(q[:,2].min());x=float(np.median(q[:,0]));y=float(np.median(q[:,1]))
        hip=Vector((x,yhip,zhip));foot=Vector((x,y,sole+.10))
        knee=Vector((x,(y+yhip)/2+(.035 if front else -.04),.34))
...
arm=bpy.data.objects.new('Kilnback - study rig',bpy.data.armatures.new('Kilnback anatomy rig'))
...
for i,point in enumerate(p):
    ...
    tail=smooth((-y-.65)/.09)*smooth((z-.40)/.18)*smooth((.13-abs(x))/.08)
    if tail>0:
    ...
    leg_blend=smooth((.64-z)/.19)*smooth((abs(x)-.065)/.06)
    # Head/tusks must never be captured by a nearby front leg's spatial region.
    leg_blend*=1-smooth((y-.31)/.09)
    leg_blend*=smooth((abs(y-.0)-.015)/.07)
    # The inner edge of each real hoof is still part of the leg. A generic
...
def pose(clip,t,duration):
    ...
    body_offset.z-=settings['stance_sink_metres']+abs(math.sin(lean))*.38
...
clips={'idle':4.0,'walk':1.6,'root':3.0,'turn':3.0,'windup':realtime['behaviours']['melee']['windup_seconds'],'release':.22}
scene.render.fps=100;arm.animation_data_create()
for clip,duration in clips.items():
    action=bpy.data.actions.new('Kilnback__'+clip);arm.animation_data.action=action
...
for label,count in [('mid',settings['mid_triangles']),('far',settings['far_triangles'])]:
    obj=near.copy();obj.data=near.data.copy();obj.name='Kilnback - '+label+' candidate';scene.collection.objects.link(obj)
...
report={'source_sha256':cfg['source_sha256'],'surface_sha256':hashlib.sha256((surface/'boar-surface.blend').read_bytes()).hexdigest(),
        'settings':settings,'bones':bones,'feet':feet,'clips_seconds':clips,'variants':[],
...
for label,obj in zip(['near','mid','far'],variants):
    ...
    fallback=bpy.data.materials.new('Kilnback - PBR fallback (bind scar shader in Godot)');fallback.use_nodes=True
    ...
    bpy.ops.export_scene.gltf(filepath=str(output/('boar-'+label+'.glb')),export_format='GLB',use_selection=True,export_yup=True,export_animations=True,export_animation_mode='NLA_TRACKS',export_frame_range=False,export_anim_slide_to_zero=True,export_cameras=False,export_lights=False)
...
print('BOAR_RIG_OK '+str(output),flush=True)
