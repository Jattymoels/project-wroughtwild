param(
    ...
    [string]$Scene = 'wide_frontier_review',
    [switch]$Import,
    [switch]$Rendered,
    [switch]$Script,
    [switch]$Prepare,
...
$project = Join-Path $repo ("build/wide-frontier/$ReviewName/game")
$logs = Join-Path $repo 'build/wide-frontier/logs'
New-Item -ItemType Directory -Path $logs -Force | Out-Null
$env:APPDATA = Join-Path $repo 'build/wide-frontier/appdata'
...
if ($Prepare) {
    ...
    $projectText = [IO.File]::ReadAllText($projectFile).Replace('[application]', "[application]`nconfig/use_custom_user_dir=true`nconfig/custom_user_dir_name=`"WroughtwildWideFrontierReview`"")
    ...
    Copy-Item -LiteralPath (Join-Path $repo 'game/bin/wroughtwild_sim.gdextension') -Destination (Split-Path $reviewLibrary) -Force
...
if (-not (Test-Path -LiteralPath (Join-Path $project 'project.godot'))) { throw 'Prepare the isolated review copy first; see docs/prototype/wide-frontier-intensive-2026-09-06.md.' }
$name = if ($Import) { 'import' } else { $Scene }
$out = Join-Path $logs ($name + '.out.log')
$err = Join-Path $logs ($name + '.err.log')
$arguments = if ($Import) { '--headless --import' } elseif ($Script) { "--headless --script res://tests/$Scene.gd" } elseif ($Rendered) { "--position -9999,-9999 res://tests/$Scene.tscn" } else { "--headless res://tests/$Scene.tscn" }
