param(
 [ValidateSet('import','before','after','lifecycle')][string]$Job='before',
...
$rfOutput="$rfRoot/build/play03-arrival"
...
try {
 ...
 if($Job -eq 'import') {
  ...
  $arguments+= $(if($Job -eq 'lifecycle'){'res://tests/play03_arrival_lifecycle.tscn'}else{'res://tests/play03_arrival.tscn'})
  if($Job -in @('before','after')) {
