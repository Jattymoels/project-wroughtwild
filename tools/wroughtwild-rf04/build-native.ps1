...
if($LASTEXITCODE -ne 0){throw 'RF04 native configure failed'}
& $rfCmake --build "$rfOut/native" -j 6 *> "$rfOut/native/build.log"
if($LASTEXITCODE -ne 0){Get-Content "$rfOut/native/build.log" -Tail 35;throw 'RF04 native build failed'}
...
$rfFiles+=@('sim/src/worldgen_frontier_v7.inc','tools/wroughtwild-rf04/CMakeLists.txt','tools/wroughtwild-rf04/build-native.ps1')
