param(
 [ValidateSet('import','diagnose','continuity','contact','walk','continue')][string]$Job='continuity',
...
$rfOutput="$rfRoot/build/rf04"
foreach($dir in @($rfOutput,"$rfOutput/user","$rfOutput/local","$rfOutput/temp","$rfOutput/logs","$rfOutput/media")) {
 New-Item -ItemType Directory -Path $dir -Force | Out-Null
}
$rfPrior=@{};foreach($key in @('APPDATA','LOCALAPPDATA','TEMP','TMP','WROUGHTWILD_RF04_OUTPUT')){$rfPrior[$key]=[Environment]::GetEnvironmentVariable($key,'Process')}
# The retained cause fixture was measured once with the inherited RF03 DLL.
# Current-source checks can reuse it; never overwrite it with the repaired mesh.
if($Job -eq 'diagnose' -and (Get-FileHash -LiteralPath "$rfRoot/game/bin/libwroughtwild_sim.windows.x86_64.dll").Hash -ne '8D65D7331E445602C48E826CB5A1D280C24AAE69B02A4C29F0AE3047CA3B65F2') {throw 'Cause diagnosis requires the inherited RF03 DLL; use continuity for the corrected worker.'}
if($Job -eq 'continuity' -and -not (Test-Path -LiteralPath "$rfOutput/cause-before.json")) {Copy-Item -LiteralPath "$rfRoot/docs/prototype/rf04-evidence-2026-09-15/cause-before.json" -Destination "$rfOutput/cause-before.json"}
$env:APPDATA="$rfOutput/user"
$env:LOCALAPPDATA="$rfOutput/local"
$env:TEMP="$rfOutput/temp"
$env:TMP="$rfOutput/temp"
$env:WROUGHTWILD_RF04_OUTPUT=$rfOutput
$mutex=$null
$held=$false
$madeOverride=$false
$process=$null
...
try {
 ...
 if($Job -eq 'import') {
  ...
  $arguments+="res://tests/rf04/$Job.tscn"
  if($Job -eq 'walk') {
