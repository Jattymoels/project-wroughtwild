...
$rfMode=if($CheckedHome){'playtest-checked-home'}else{'playtest-new'}
$rfState="$rfRoot/build/rf04/$rfMode"
...
try {
 ...
 if($CheckedHome) {
  ...
  if(-not (Test-Path -LiteralPath $rfSave)) {
   ...
   Copy-Item -LiteralPath "$rfRoot/build/rf04/private-world.json" -Destination $rfSave
  }
  Write-Output 'Choose Continue saved world to enter the checked, paid overlook footprint. Later saves stay in this private playtest folder.'
