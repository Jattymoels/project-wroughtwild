...
frames[0].save(dst/'nymph-motion.webp',save_all=True,append_images=frames[1:],duration=durations,loop=0,quality=78,method=4)
with Image.open(dst/'nymph-motion.webp') as clip:
...
(dst/'media.json').write_text(json.dumps({'captured_frames':270,'encoded_frames':encoded_frames,'duration_seconds':9,'dimensions':[640,480],'renderer':'Forward+','source':'native Enemy in flat capture fixture, fixed 60Hz physics; 30fps selected frames','segments':{'0-1.5':'native idle','1.5-5':'native pursuit of scripted moving target','5-8':'native windup and root strikes','8-9':'native freeze'},'path':'nymph-motion.webp','bytes':(dst/'nymph-motion.webp').stat().st_size},indent=2)+'\n')
print('MEDIA_OK', (dst/'nymph-motion.webp').stat().st_size)
