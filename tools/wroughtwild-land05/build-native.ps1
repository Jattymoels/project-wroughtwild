...
if($LASTEXITCODE -ne 0){throw 'LAND05 native configure failed'}
...
if($rfBuild.ExitCode -ne 0){Get-Content "$rfOut/native/build.log","$rfOut/native/build.err.log" -Tail 35;throw 'LAND05 native build failed'}
...
$rfFiles+=@('sim/src/worldgen_frontier_v7.inc','tools/wroughtwild-land05/CMakeLists.txt','tools/wroughtwild-land05/build-native.ps1')
...
Write-Output "LAND05_BUILT $rfDll"
