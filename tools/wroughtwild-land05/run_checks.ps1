param(
 [ValidateSet('import','diagnostic','use','continue','early')][string]$Job='diagnostic',
...
$rfOutput="$rfRoot/build/land05"
foreach($dir in @($rfOutput,"$rfOutput/user","$rfOutput/local","$rfOutput/temp","$rfOutput/logs","$rfOutput/media")) {
 New-Item -ItemType Directory -Path $dir -Force | Out-Null
}
$rfPrior=@{};foreach($key in @('APPDATA','LOCALAPPDATA','TEMP','TMP','WROUGHTWILD_LAND05_OUTPUT')){$rfPrior[$key]=[Environment]::GetEnvironmentVariable($key,'Process')}
# Focused LAND05 phases use only private LAND05 fixtures and output directories.
$env:APPDATA="$rfOutput/user"
$env:LOCALAPPDATA="$rfOutput/local"
$env:TEMP="$rfOutput/temp"
$env:TMP="$rfOutput/temp"
$env:WROUGHTWILD_LAND05_OUTPUT=$rfOutput
$mutex=$null
$held=$false
$madeOverride=$false
$process=$null
...
if($Job -eq 'use') {
 $reference=(& git -c "safe.directory=$rfRoot" -C $rfRoot show '0240b5a5561c46084848f152c0ab464b71b2723d:game/scripts/ground_cover.gd') -join "`n"
 $reference=$reference.Replace("class_name GroundCover`n",'')
 [System.IO.File]::WriteAllText("$rfRoot/game/tests/land05/baseline_ground.gd",$reference,[System.Text.UTF8Encoding]::new($false))
}
$started=Get-Date
$stamp=$started.ToString('yyyyMMdd-HHmmss')
$rfLabel=$Job
$log="$rfOutput/logs/$rfLabel-$stamp"
try {
 $arguments=@('--path',"$rfRoot/game",'--audio-driver','Dummy')
 if($Job -eq 'import') {
  $arguments+=@('--headless','--editor','--import')
 } else {
  $arguments+="res://tests/land05/$Job.tscn"
  if($Job -in @('use','early','diagnostic','continue')) {
   $mutex=[System.Threading.Mutex]::new($false,'Local\Wroughtwild-Art07-GPU')
 ...
 while(-not $process.WaitForExit(5000)) {
  ...
  if($waits -ge 108) {throw "Owned $Job reached its focused-run timeout."}
 }
 $process.WaitForExit()
 ...
 @{exitCode=$resultCode;engineExitCode=$process.ExitCode;seconds=[Math]::Round(((Get-Date)-$started).TotalSeconds,2);reportedErrors=$errors.Count;log=$log} | ConvertTo-Json | Set-Content -LiteralPath "$rfOutput/$rfLabel-result.json" -Encoding utf8
 Get-Content -LiteralPath "$rfOutput/$rfLabel-result.json"
