param([string]$Revision='f7253d20ed1dbeee743219190bdaccac60921c1c', [Parameter(Mandatory)][string]$Output)
$ErrorActionPreference='Stop'
$artRepo=[IO.Path]::GetFullPath((Join-Path $PSScriptRoot '../..'))
...
& git -C $artRepo archive --format=zip --output="$artOutput/baseline.zip" $Revision sim data game/extensions/wroughtwild_sim game/bin/wroughtwild_sim.gdextension
...
project(art04_native LANGUAGES CXX)
...
& cmake --build "$artOutput/compile" -j 6 *> "$artOutput/build.log"
...
Write-Output 'ART04_NATIVE_BUILD_OK'
