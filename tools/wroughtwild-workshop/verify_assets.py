...
for kind,limit in [('source',np.array([1.5,1.5,1.1])),('buffer',np.array([1,1,1.2]))]:
    for lod in ['near','mid','far']:
        path=assets/f'red-{kind}-{lod}.glb'
...
for i in range(1,4):
    path=assets/f'red-fragment-{i}.glb'
    bpy.ops.wm.read_factory_settings(use_empty=True);bpy.ops.import_scene.gltf(filepath=str(path))
    ...
    bm.free()
bpy.ops.wm.open_mainfile(filepath=str(assets/'red-workshop.blend'))
check('SOURCE - untouched normalized Red inclusion' in bpy.data.objects,'dense untouched source retained')
check('SOURCE - editable procedural housing' in bpy.data.objects,'editable material authoring retained')
textures=[(name,2048) for name in ['red-base.png','red-orm.png','red-normal.png','red-scar.png','buffer-base.png','buffer-orm.png','buffer-normal.png']]
...
host=bpy.data.objects['Red inclusion - ready worked or spent host'];host.location.x=-1.25
housing=bpy.data.objects['BUFFER_HOUSING - paid crafted frame'];housing['export_location']=list(housing.location);housing.location.x+=1.1
core=bpy.data.objects['RED_CORE - fitted inclusion'];core['export_location']=list(core.location);core.location.x+=1.1
shown=[host,housing,core]
for i in range(3):
    o=bpy.data.objects['Red Salt - cut fragment '+str(i+1)];o.location=(i*.35-.35,-.8,0);shown.append(o)
...
bpy.ops.wm.save_as_mainfile(filepath=str(out/'red-workshop.blend'),compress=True)
scene.render.filepath=str(out/'editable-preview.png');bpy.ops.render.render(write_still=True)
bpy.ops.wm.open_mainfile(filepath=str(out/'red-workshop.blend'))
for im in bpy.data.images:
...
report={'checks':checks,'failures':[],'runtime_assets':records,'blender':bpy.app.version_string,'packed_blend_sha256':hashlib.sha256((out/'red-workshop.blend').read_bytes()).hexdigest()}
