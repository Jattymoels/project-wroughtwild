# ART-07R6 — Resolve faceted-ore visual coverage

Copy the full fenced prompt into one task using the working directory below.

```text
Implement ART-07R6 only: Resolve faceted-ore visual coverage.
Working directory: D:/project-wroughtwild-art07-r6
Branch: codex/art07-r6
...
- tools/wroughtwild-art07/c5/README.md
- tools/wroughtwild-art07/c5/native_resource.gd
- docs/systems/world-generation.md
- docs/systems/construction.md

OUTCOME
Provide a coherent existing-ore presentation on ordinary faceted terrain within its retained surface and excavation envelope.

FINDINGS / CONTEXT
G2-V04
Original source-owner context: C5, G1.
...
OWNED GIT OUTPUTS
- tools/wroughtwild-art07-repairs/r6/
- docs/art/leyline-studies/2026-09-14/art07-repairs/r6/
- docs/prototype/art07-repairs/2026-09-14/receipts/r6.md
- docs/prototype/art07-repairs/2026-09-14/receipts/r6.json
Ignored work: D:/project-wroughtwild-art07-r6/build/art07-repairs/r6/<fresh-version>/.

SETUP
From this worktree run the installed Python with tools/wroughtwild-art07-repairs/workspace.py inspect --id r6.
Before source consumption, run workspace.py verify --id r6; retain its full input-hash results.
Inspect build/art07-repairs/r6/v01/prepared.json and reuse the matching prepared
runtime if present. For a new absent version, run the assigned workspace.py
prepare --id r6 --version vNN. It hashes the copied runtime entries.
...
BOUNDED WORK
1. Trace the current fallback-only C5 adapter and measure the ordinary faceted ribbon, exposed surface and work/body constraints. Review available source candidates before designing changes.
2. Implement a surface-conforming visual treatment only where the existing terrain/body contract permits it. Retain the fallback candidate and make full/partial/exhausted appearances truthful.
3. Preserve every native resource ID, pose, stock, excavation boundary, targeting/collision and save record. A raised slab is not automatically valid on faceted terrain.
4. If the selected appearance cannot fit without changing terrain/colliders, deliver measured options and the exact missing decision; leave that part unapproved. Do not label fallback-only coverage as a completed faceted-terrain repair.

REQUIRED EVIDENCE
- Render native faceted and fallback cases separately at player height and close range, including slopes/edges, partial extraction, exhaustion and exposed excavation in both renderers.
- Replay unchanged C5 native harvest/partial-restart assertions and compare full art-on/off resource/geography ownership.
- Prove no buried/airborne visual slab, phantom solid surface or mismatched remaining stock is introduced. Reopen changed source masters and report costs.
