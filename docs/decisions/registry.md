...
| D-004 | Classes | ~~Starting class leads toward a guaranteed class hall providing another vertical progression path~~ Amended 4 Sep 2026 (D-023 slice 9): no class at the start - the bare Foundry plate is the base class; the first class hall's test grants a specialisation that opens the plate's exterior (rails), and later halls add exterior options. The Tyrant's forge stands in for the hall until it is built | Accepted direction | The class hall is built as an authored module |
...
- **D-023 (3 Sep 2026, the plate as a working):** the owner, on the
  ...
  **Slice 8 landed 4 Sep 2026:** the Marrow's eight sustain forms and the
  ...
  56.
  **Slice 9 landed 4 Sep 2026**, on the owner's answer that amends D-004
  ("base tablet setups for the base class coming from nothing ... first
  class hall test completion gives you a choice for specialising and that
  changes the exterior of the rows/columns upgrades"): rails as the
  plate's exterior. No class at the start; the Tyrant's forge stands in
  for the first hall's test and its completion offers the specialisation
  once - Ranger, Warden or Kindler, two patterns each; a pattern set in a
  rail reads its whole row or column and bends its rule while the line
  holds (one rail in era two, two in era three, one per pattern); the
  Hound's and the Husk's manners are taught by kills and join them. The
  engine fans projectiles, pierces, counts armour against the elements,
  doubles and staggers the Barbs, spreads ignites on the hit, makes
  burning ground heal, punishes approach and rewards stillness. Sim
  tests 3578; engine unit 356, integration 220, grammar 62.
