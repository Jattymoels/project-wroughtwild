...
MachineWorld::MachineWorld(Config config, WorldIdentity identity) : config_(std::move(config)), identity_(std::move(identity)) {
    ...
    if (!identity_.sources.empty() && identity_.profile != "frontier_v5" && identity_.profile != "frontier_v6" && identity_.profile != "frontier_v7" && !leyline::supports(identity_.profile)) throw std::runtime_error("contraptions: pressure belongs only to its frozen source profile");
...
std::map<std::string, State> MachineWorld::parse(const std::string& source, std::map<std::string,int>* stocks) const {
    ...
    if (schema==1) {
        if (identity_.profile=="frontier_v5" || identity_.profile=="frontier_v6" || identity_.profile=="frontier_v7" || !identity_.sources.empty()) throw std::runtime_error("contraptions: source world requires its complete pressure ledger");
