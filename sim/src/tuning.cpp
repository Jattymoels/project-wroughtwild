...
WorldgenTable loadWorldgen(const std::string& path) {
    ...
    if (table.generationProfile == "frontier_v13") {
        table.ambientElkKeepFraction = doc->get("ambient_elk_keep_fraction").asNumber();
        if (!std::isfinite(table.ambientElkKeepFraction) || table.ambientElkKeepFraction<0 || table.ambientElkKeepFraction>1)
            throw std::runtime_error("LAND05: ambient elk keep fraction must be finite and between zero and one");
    }
    ...
    if(auto wide=doc->find("wide_frontier")) {
        auto& w=table.wideFrontier;
        const auto value=[&](const std::string& key,double low,double high){const double v=wide->get(key).asNumber();if(!std::isfinite(v)||v<low||v>high)throw std::runtime_error("worldgen: invalid wide frontier "+key);return v;};
        w.quietRadiusM=value("quiet_radius_m",140,160);
        w.hostileBoundaryM=value("hostile_boundary_m",190,240);
        w.firstSiegeNight=static_cast<int>(value("first_siege_night",3,3));
        w.homeCount=static_cast<int>(value("home_count",4,4));
        w.homeRadiusM=value("home_radius_m",12,16);
        w.homeSkirtM=value("home_skirt_m",12,30);
        w.homeDistanceM=value("home_distance_m",70,115);
        w.valleyBaseHeight=static_cast<int>(value("valley_base_height",12,40));
        w.valleyReliefCells=static_cast<int>(value("valley_relief_cells",4,12));
        w.valleyFrequency=value("valley_frequency",0.006,0.018);
        w.valleyTransitionM=value("valley_transition_m",30,60);
        w.rollingFrequency=value("rolling_frequency",0.01,0.025);
        w.crossFrequency=value("cross_frequency",0.008,0.02);
        w.populationKeep=value("population_keep",0.1,0.5);
        w.homeTrees=static_cast<int>(value("home_trees",4,8));
        w.homeBoulders=static_cast<int>(value("home_boulders",3,5));
    ...
    } else if((table.generationProfile=="frontier_v6" || (table.generationProfile=="frontier_v7" || (table.generationProfile=="frontier_v8" || (table.generationProfile=="frontier_v9" || (table.generationProfile=="frontier_v10" || (table.generationProfile=="frontier_v11" || (table.generationProfile == "frontier_v12" || table.generationProfile == "frontier_v13"))))))))throw std::runtime_error("worldgen: V6 needs explicit opening inputs");

    if((table.generationProfile=="frontier_v6" || (table.generationProfile=="frontier_v7" || (table.generationProfile=="frontier_v8" || (table.generationProfile=="frontier_v9" || (table.generationProfile=="frontier_v10" || (table.generationProfile=="frontier_v11" || (table.generationProfile == "frontier_v12" || table.generationProfile == "frontier_v13"))))))) && (table.map.widthCells!=1024 || table.map.heightCells!=1024 || table.map.worldDepth!=96 || table.map.cellSizeM!=1.0))
        throw std::runtime_error("worldgen: V6 requires its accepted 1024x1024x96 one-metre extent");

    if ((table.generationProfile == "frontier_v7" || (table.generationProfile == "frontier_v8" || (table.generationProfile=="frontier_v9" || (table.generationProfile=="frontier_v10" || (table.generationProfile=="frontier_v11" || (table.generationProfile == "frontier_v12" || table.generationProfile == "frontier_v13"))))))) {
        ...
        const auto value=[&](const std::string& key,double low,double high) {
            ...
            if(!std::isfinite(v)||v<low||v>high)throw std::runtime_error("worldgen: invalid reclaimed frontier "+key);
            return v;
        };
        r.landformAmplitudeM=value("landform_amplitude_m",0,8);
        r.landformWavelengthM=value("landform_wavelength_m",100,240);
        r.homeDistanceJitterM=value("home_distance_jitter_m",0,8);
        r.homeAngleJitter=value("home_angle_jitter",0,.2);
        r.homeBlendM=value("home_blend_m",24,36);
        r.extensionM=value("extension_m",4,12);
        r.approachSmoothingM=value("approach_smoothing_m",2,5);
        r.bankHeightM=value("bank_height_m",2,6);
        r.overlookLiftM=value("overlook_lift_m",3,6);
        r.woodlandRadiusM=value("woodland_radius_m",50,68);
        r.woodlandDensity=value("woodland_density",.015,.04);
        r.impactShoulderM=value("impact_shoulder_m",1,4);
        if(table.wideFrontier.homeRadiusM!=14)throw std::runtime_error("worldgen: RF-03 retains its 14m home core");
    }

    if((table.generationProfile=="frontier_v8" || (table.generationProfile=="frontier_v9" || (table.generationProfile=="frontier_v10" || (table.generationProfile=="frontier_v11" || (table.generationProfile == "frontier_v12" || table.generationProfile == "frontier_v13")))))) {
    ...
    if((table.generationProfile=="frontier_v9" || (table.generationProfile=="frontier_v10" || (table.generationProfile=="frontier_v11" || (table.generationProfile == "frontier_v12" || table.generationProfile == "frontier_v13"))))) {
        const auto& input=doc->get("scarwater"); auto& s=table.scarwater;
        const auto val=[&](const std::string& key,double lo,double hi) {
            const double v=input.get(key).asNumber();
            if(!std::isfinite(v)||v<lo||v>hi)throw std::runtime_error("worldgen: invalid Scarwater "+key);
            return v;
        };
        s.ridgeHalfLengthM=val("ridge_half_length_m",60,100);s.ridgeReliefM=val("ridge_relief_m",20,35);
        s.ridgeWidthM=val("ridge_width_m",28,55);s.fissureWidthM=val("fissure_width_m",4,(table.generationProfile=="frontier_v10" || (table.generationProfile=="frontier_v11" || (table.generationProfile == "frontier_v12" || table.generationProfile == "frontier_v13")))?10:8);
        s.fissureDepthM=val("fissure_depth_m",2,4);s.fissureLengthM=val("fissure_length_m",14,26);
        s.woodlandBandM=val("woodland_band_m",15,35);s.routeWidthM=val("route_width_m",4,7);
        s.candidateCount=static_cast<int>(val("candidate_count",8,32));
        s.lakeAspectDelta=val("lake_aspect_delta",0,.18);
    }
    if((table.generationProfile=="frontier_v10" || (table.generationProfile=="frontier_v11" || (table.generationProfile == "frontier_v12" || table.generationProfile == "frontier_v13")))) {
        const auto& input=doc->get("scarwater"); auto& s=table.scarwater;
        s.fractureSpacingM=input.get("fracture_spacing_m").asNumber();
        s.fractureJointM=input.get("fracture_joint_m").asNumber();
        s.fractureLean=input.get("fracture_lean").asNumber();
        if(!std::isfinite(s.fractureSpacingM)||s.fractureSpacingM<12||s.fractureSpacingM>25 ||
           !std::isfinite(s.fractureJointM)||s.fractureJointM<1.5||s.fractureJointM>5 ||
           !std::isfinite(s.fractureLean)||s.fractureLean<.1||s.fractureLean>.5)
            throw std::runtime_error("worldgen: invalid V10 fracture inputs");
    }
    if((table.generationProfile=="frontier_v11" || (table.generationProfile == "frontier_v12" || table.generationProfile == "frontier_v13"))) {
        const auto& input=doc->get("dry_steppe"); auto& s=table.drySteppe;
        const auto val=[&](const std::string& key,double lo,double hi) {
            const double v=input.get(key).asNumber();
            if(!std::isfinite(v)||v<lo||v>hi)throw std::runtime_error("worldgen: invalid Dry Steppe "+key);
            return v;
        };
        s.centreDistanceM=val("centre_distance_m",160,220);
        s.halfLengthM=val("half_length_m",110,160);s.halfWidthM=val("half_width_m",85,140);
        s.rollingReliefM=val("rolling_relief_m",2,6);s.ribHeightM=val("rib_height_m",4,9);
        s.ribSpacingM=val("rib_spacing_m",20,36);s.cutDepthM=val("cut_depth_m",1,3);
        s.routeWidthM=val("route_width_m",5,9);s.homeBankM=val("home_bank_m",2,5);
        s.sourceWorkRadiusM=val("source_work_radius_m",6,10);
        s.candidateCount=static_cast<int>(val("candidate_count",8,48));
    }
    if((table.generationProfile == "frontier_v12" || table.generationProfile == "frontier_v13")) {
        const auto& input=doc->get("force_journeys"); auto& s=table.forceJourneys;
        const auto val=[&](const std::string& key,double lo,double hi) {
            const double v=input.get(key).asNumber();
            if(!std::isfinite(v)||v<lo||v>hi)throw std::runtime_error("worldgen: invalid force journey "+key);
            return v;
        };
        s.radiusM=val("radius_m",24,38);s.approachLengthM=val("approach_length_m",20,32);
        s.routeWidthM=val("route_width_m",5,8);s.workRadiusM=val("work_radius_m",5,8);
        s.whiteReliefM=val("white_relief_m",2,5);s.blueReliefM=val("blue_relief_m",2,5);
        s.greenReliefM=val("green_relief_m",.4,1.4);
        s.primarySearchM=val("primary_search_m",0,340);s.fallbackSearchM=val("fallback_search_m",360,470);
        s.sourceSeparationM=val("source_separation_m",60,100);
        s.candidateStrideM=static_cast<int>(val("candidate_stride_m",2,8));
    }
    ...
    if (auto regions = doc->find("regions")) {
        for (const auto& entry : regions->asArray()) {
            ...
            if (r.id.empty() || !regionIds.insert(r.id).second || !table.findBiome(r.biome) ||
                !std::isfinite(r.radiusM) || r.radiusM < 40 || r.radiusM > ((table.generationProfile == "frontier_v6" || (table.generationProfile == "frontier_v7" || (table.generationProfile == "frontier_v8" || (table.generationProfile=="frontier_v9" || (table.generationProfile=="frontier_v10" || (table.generationProfile=="frontier_v11" || (table.generationProfile == "frontier_v12" || table.generationProfile == "frontier_v13"))))))) ? 160 : 70) ||
                !std::isfinite(r.transitionM) || r.transitionM < 10 || r.transitionM > ((table.generationProfile == "frontier_v6" || (table.generationProfile == "frontier_v7" || (table.generationProfile == "frontier_v8" || (table.generationProfile=="frontier_v9" || (table.generationProfile=="frontier_v10" || (table.generationProfile=="frontier_v11" || (table.generationProfile == "frontier_v12" || table.generationProfile == "frontier_v13"))))))) ? 60 : 30) ||
                ...
                r.reliefCells < 0 || r.reliefCells > ((table.generationProfile == "frontier_v6" || (table.generationProfile == "frontier_v7" || (table.generationProfile == "frontier_v8" || (table.generationProfile=="frontier_v9" || (table.generationProfile=="frontier_v10" || (table.generationProfile=="frontier_v11" || (table.generationProfile == "frontier_v12" || table.generationProfile == "frontier_v13"))))))) ? 40 : 8))
    ...
    } else if (table.generationProfile == "frontier_v4" || table.generationProfile == "frontier_v5" || (table.generationProfile == "frontier_v6" || (table.generationProfile == "frontier_v7" || (table.generationProfile == "frontier_v8" || (table.generationProfile=="frontier_v9" || (table.generationProfile=="frontier_v10" || (table.generationProfile=="frontier_v11" || (table.generationProfile == "frontier_v12" || table.generationProfile == "frontier_v13")))))))) {
    ...
    } else if (table.generationProfile == "frontier_v5" || (table.generationProfile == "frontier_v6" || (table.generationProfile == "frontier_v7" || (table.generationProfile == "frontier_v8" || (table.generationProfile=="frontier_v9" || (table.generationProfile=="frontier_v10" || (table.generationProfile=="frontier_v11" || (table.generationProfile == "frontier_v12" || table.generationProfile == "frontier_v13")))))))) {
...
Tuning loadAll(const std::string& tuningDirectory) {
    ...
    tuning.frontierV10Worldgen = loadWorldgen(tuningDirectory + "/worldgen-frontier-v10.json");
    tuning.frontierV11Worldgen = loadWorldgen(tuningDirectory + "/worldgen-frontier-v11.json");
    tuning.frontierV12Worldgen = loadWorldgen(tuningDirectory + "/worldgen-frontier-v12.json");
    tuning.frontierV13Worldgen = loadWorldgen(tuningDirectory + "/worldgen-frontier-v13.json");
    tuning.frontierV9Worldgen = loadWorldgen(tuningDirectory + "/worldgen-frontier-v9.json");
    ...
    tuning.frontierV5Worldgen = loadWorldgen(tuningDirectory + "/worldgen-frontier-v5.json");
