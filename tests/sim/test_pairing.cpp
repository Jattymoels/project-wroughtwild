...
int main(int argc,char** argv) {
    ...
    for(int mode=0;mode<2;++mode) {
        ...
        check(!player.worldEffectActive("lf5_pairing_victory") && !player.curioHeld("warden_eye") && player.currentEra()==2 && player.resonanceState.toJson()==first,"death/abandon never pays first-clear or changes first event");
    }
    ...
    for(int repeat=0;repeat<2;++repeat) {
        ...
        session->abandon();
        check(player.currentEra()==2 && player.resonanceState.toJson()==first,"LF5B cannot publish an era or replace first ledger");
