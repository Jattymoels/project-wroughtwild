// RF-03: only primary 77 (repeated) and variation 904. Existing wide-world
// guarantee helpers are reused; no historical seed or renderer sweep.
...
uint64_t exactFingerprint(const worldgen::WorldMap& map){ExactWorldFingerprint h;return h.world(map);}
...
void wide(const tuning::Tuning& t,uint64_t seed,bool repeat) {
    ...
    const auto m=worldgen::generateProfile(t,seed,"frontier_v7");
    ...
    check(std::hypot(m.gateX-m.spawnX,m.gateZ-m.spawnZ)*m.cellSize>=t.frontierV7Worldgen.guarantees.gateMinDistanceM,"gate fallback moved progression into the opening");
    ...
    for(const auto& r:m.regions){
        ...
        for(int z=r.z-int(r.radiusM*.85);z<=r.z+int(r.radiusM*.85);z+=2)for(int x=r.x-int(r.radiusM*.85);x<=r.x+int(r.radiusM*.85);x+=2)if(std::hypot(x-r.x,z-r.z)<r.radiusM*.85){
            ++cells;if(t.frontierV7Worldgen.biomes[m.at(x,z).biomeIndex].id==r.biome)++typed;
        ...
        const auto& biome=*t.frontierV7Worldgen.findBiome(r.biome);
    ...
    for(const auto& r:m.ruins){clearRoute(m,r.approach,occupied);clearRoute(m,r.discoveryRoute,occupied,false);
        if(r.kind=="forge_threshold")check(std::hypot(r.x-m.gateX,r.z-m.gateZ)*m.cellSize<=t.frontierV7Worldgen.cataclysm.thresholdSearchM,"threshold is detached from its Forge gate");
    }
    ...
    const auto& a=m.homeSites[0]; const auto& b=m.homeSites[1];
    check(a.x!=b.x||a.z!=b.z,"distinct home settings");
    int forest=0,expanded=0;
    for(int dz=-50;dz<=50;++dz)for(int dx=-50;dx<=50;++dx){
        if(std::hypot(dx,dz)<50&&t.frontierV7Worldgen.biomes[m.at(a.x+dx,a.z+dz).biomeIndex].id=="forest")++forest;
        const auto& terrace=m.homeSites[2];
        if(std::hypot(dx,dz)>14&&std::hypot(dx,dz)<25&&m.topSolid(terrace.x+dx,terrace.z+dz)==terrace.y)++expanded;
    }
    check(forest>1200,"woodland pocket lacks a wooded margin");
    check(expanded>100,"terrace has no useful extension beyond the core");
    for(size_t i=0;i<m.homeSites.size();++i){const auto& h=m.homeSites[i];
        std::cout<<"\nRF03_HOME "<<seed<<" "<<i<<" "<<h.x<<","<<h.y<<","<<h.z<<" route="<<h.approach.size();
    }
    if(repeat){auto other=t;std::reverse(other.frontierV7Worldgen.regions.begin(),other.frontierV7Worldgen.regions.end());std::reverse(other.frontierV7Worldgen.habitats.begin(),other.frontierV7Worldgen.habitats.end());std::reverse(other.frontierV7Worldgen.rareSites.begin(),other.frontierV7Worldgen.rareSites.end());
        check(exactFingerprint(m)==exactFingerprint(worldgen::generateProfile(other,seed,"frontier_v7")),"V6 is not deterministic under definition reordering");
    }
    std::cout<<"\nWIDE_SEED "<<seed<<" seconds="<<seconds<<" nodes="<<m.nodes.size()<<" packs="<<m.packs.size()<<" hash="<<exactFingerprint(m)<<std::endl;
}
int main(int argc,char** argv){try{
    const std::string dir=argc>1?argv[1]:"data/tuning";
    tuning::Tuning t;t.frontierV7Worldgen=tuning::loadWorldgen(dir+"/worldgen-frontier-v7.json");
    check(worldgen::knownProfile("frontier_v7")&&!worldgen::knownProfile("frontier_v8"),"strict profile allowlist");
    wide(t,77,true);wide(t,904,false);
    std::cout<<"RF03_NATIVE "<<checks<<" checks passed\n";return 0;
}catch(const std::exception& e){std::cerr<<"FAIL RF03: "<<e.what()<<std::endl;return 1;}}
