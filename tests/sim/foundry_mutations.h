...
void testFoundryMutations(const tuning::Tuning& t) {
    ...
    for(const auto& form:t.foundry.forms) {
        if(form.supportOnly || !form.upstreamKind.empty()) continue;
        ++baseForms;
    ...
    for(const auto& metal:t.foundry.metals) for(const auto& skill:t.skills.combatSkills) {
        ...
        const auto key=kind.id=="frost_catalyst" && ingot.id=="ember" ? "smoulder_slow" : signature.at(kind.id);
        const double expected = compatible ? (kind.id=="frost_catalyst" && ingot.id=="ember" ? .25 : baseline.at(kind.id)) : 0;
    ...
    int spreadReadings=0;
    for(const auto& e:readings) if(e.modifier=="mutation_ignite_spread") ++spreadReadings;
    check(spreadReadings==6,"routes: six traversed ingots read once, even after two branches merge");
    std::reverse(branched.plate.begin(),branched.plate.end());
    int reversed=0;
    for(const auto& e:foundry::effects(t,branched,larger)) if(e.modifier=="mutation_ignite_spread") ++reversed;
